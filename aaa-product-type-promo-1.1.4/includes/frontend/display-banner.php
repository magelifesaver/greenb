 ...
 * category will only display in that category's archive.  If a link URL
 * is provided via the meta box, the image is wrapped in an anchor.
 */
...
add_action( 'wp_head', function () {
    ...
    <style>
        ...
        /* Ensure anchor wrappers fill the banner area so clicking anywhere triggers the link. */
        .aaa-promo-banner .aaa-promo-link {
            display: block;
            width: 100%;
            /* Let height be determined by its contents (the image). */
        }
    </style>
    <?php
} );