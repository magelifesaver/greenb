	...
	var Timepicker = /*#__PURE__*/ function() {
		...
		_createClass(Timepicker, [{
			...
			value: function _setTimeValue(value, source) {
				if (this.targetEl.nodeName === "INPUT") {
					...
					if (settings.useSelect && source != "select" && tp.list) {
						tp.list.val(tp._roundAndFormatTime(tp.anytime2int(value, false)));
					}
				}
				...
				if (this.selectedValue != value) {
					...
					// Arosoft 20240924 We have to trigger INPUT event to get react input to read new time value
					var eventInput = new Event('input', 
						EVENT_DEFAULTS);
						this.targetEl.dispatchEvent(eventInput);
			...
			value: function _selectValue() {
				...
				var timeValue_end = null;
				var step = tp.settings.step; // Added by arosoft for displaying range in input form
				 step = parseInt(step);  // Added by arosoft for displaying range in input form
				...
				if (timeValue !== null) {
					// Added by arosoft for displaying range in input form
					if (typeof step != "string" && !isNaN(step)) {
						
						let end  = parseInt(timeValue)  + (step*60);
						timeValue_end = tp._int2time(end);
					}
					...
					// Added by arosoft for displaying range in input form
					if (tp.settings.showDuration && !_.isNull(timeValue_end)) {
						timeString = timeValue+ ' - ' +timeValue_end;
					}else{
						timeString = timeValue;
					}
					tp._setTimeValue(timeString, "select");
				}
				return true;
			}
		}, {
			key: "anytime2int",
			value: function anytime2int(input, from_args) {
				if (typeof input === 'number') {
					return input;
				} else if (typeof input === 'string') {
					return this.time2int(input, from_args);
			...
			key: "time2int",
			value: function time2int(timeString, from_args) {
				if (timeString === "" || timeString === null || timeString === undefined) {
					return null;
				}
				if (timeString === 'now') {
					// return this.anytime2int(new Date(), false);
					
					return parseInt(this.settings.secondsfromMidnight) + parseInt(this.settings.extra);
				...
				// Arosoft adds variable 'from_args' because format on time from arguments is always 'h:i'
				if (!from_args && hour < 12 && !ampm && this.settings._twelveHourTime && this.settings.scrollDefault()) {
			...
			value: function intStringDateOrFunc2func(input, from_args) {
				var _this = this;
				if (input === null || input === undefined) {
					...
					return function() {
						return _this.anytime2int(input(), from_args);
					};
				} else {
					return function() {
						return _this.anytime2int(input, from_args);
			...
			value: function parseSettings(settings) {
				...
				// Add by Arosoft
				if (settings.extra) {
					settings.extra = parseInt(settings.extra);
				} else {
					settings.extra = 0;
				}
				if (settings.listWidth) {
					settings.listWidth = this.anytime2int(settings.listWidth, false);
				}
				settings.minTime = this.intStringDateOrFunc2func(settings.minTime, true);
				settings.maxTime = this.intStringDateOrFunc2func(settings.maxTime, true);
				settings.durationTime = this.intStringDateOrFunc2func(settings.durationTime, true);
				if (settings.scrollDefault) {
					settings.scrollDefault = this.intStringDateOrFunc2func(settings.scrollDefault, true);
				} else {
				...
				if (settings.disableTimeRanges.length > 0 || settings.extra != 0) {
					// convert string times to integers
					for (var i in settings.disableTimeRanges) {
						settings.disableTimeRanges[i] = [this.anytime2int(settings.disableTimeRanges[i][0], true), this.anytime2int(settings.disableTimeRanges[i][1], true)];
					}
					if (settings.extra != 0 && (this.anytime2int(this.settings.scrollDefault(), false) + parseInt(settings.time_to_add) * 60) > this.anytime2int(settings.minTime, true)) {
						settings.disableTimeRanges.push([0, this.anytime2int(this.settings.scrollDefault(), false) + parseInt(settings.time_to_add) * 60]);
					}
			...
			value: function _setSelected() {
				...
				var timeValue = this.anytime2int(this._getTimeValue(), false);
				if (timeValue === null) {
					return;
				}
			...
			value: function _formatValue(e, origin) {
				...
				var seconds = this.anytime2int(this.targetEl.value, false);
				if (seconds === null) {
	...
	(function(factory) {
		...
		var methods = {
			...
			show: function show(e) {
				...
				if (!selected.length) {
					var timeInt = tp.anytime2int(tp._getTimeValue(), false);
					if (timeInt !== null) {
			...
			getSecondsFromMidnight: function getSecondsFromMidnight() {
				var tp = this[0].timepickerObj;
				return tp.anytime2int(tp._getTimeValue(), false);
			},
			getTime: function getTime(relative_date) {
				...
				var offset = tp.anytime2int(time_string, false);
				if (offset === null) {
					return null;
				}
				if (!relative_date) {
			...
			setTime: function setTime(value) {
				...
				var settings = tp.settings;
				if (settings.forceRoundTime) {
					var prettyTime = tp._roundAndFormatTime(tp.anytime2int(value), false);
				} else {
					var prettyTime = tp._int2time(tp.anytime2int(value, false));
				}
		...
		function _render(self) {
			...
			for (var i = start, j = 0; i <= end; j++, i += stepFunc(j) * 60) {
				...
				if ((settings.minTime() !== null || settings.durationTime() !== null) && settings.showDuration) {
					var durationString = tp._int2time(timeInt + settings.step * 60); // tp._int2duration(i - durStart, settings.step);
					if (settings.useSelect) {
			...
			if (settings.useSelect) {
				if (self.val()) {
					list.val(tp._roundAndFormatTime(tp.anytime2int(self.val(), false)));
				}
