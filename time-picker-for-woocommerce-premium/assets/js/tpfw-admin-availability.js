		...
		var $tbody = $('.wc-shipping-class-rows'),
			...
			ShippingClassView = Backbone.View.extend({
				...
				initialize: function() {
					...
					$(document.body).on('click', '.woocommerce-save-button[type=submit]', {
						view: this
					}, this.onSubmit);
				},
				block: function() {
				...
				initRow: function(rowData) {
					...
					// Add option for tags and cats
					_.each(tpfwAvailibilityLocalizeScript.categories, function(el) {
						$tr.find('.wc-shipping-class-cats').find('select.tpfw-multiple-select').append('<option value="' + el.cat_ID + '">' + el.name + '</option>');
					});
					_.each(tpfwAvailibilityLocalizeScript.tags, function(el) {
					...
					$tr.find('.wc-shipping-class-time_from').find('input').on('change', {
						view: this
					}, this.onEditTime);
				...
				onEditTime: function(e) {
					let min = $(e.currentTarget).val();
					$(e.currentTarget).parents('td.wc-shipping-class-time_from').next('td.wc-shipping-class-time_to').find('input').attr('min', min);
				},
				onSubmit: function(event) {
						...
						if ($(element).data('id') != undefined) {
							...
							_.each($(element).find('td'), function(element2) {
								if ($(element2).find('input').length) {
									let temp = [];
									temp.push(
										$(element2).find('input').data('attribute'),
										$(element2).find('input').val()
									);
									td.push(temp);
								} else if ($(element2).find('select').length) {
									let temp = [];
									temp.push(
										$(element2).find('select').data('attribute'),
										$(element2).find('select').val()
									);
									td.push(temp);
								}
							});
					...
					document.getElementById('tpfwava').value = JSON.stringify(rows);
				},
				...
				setUnloadConfirmation: function() {
					...
					$save_button.prop('selected', false);
				},
...
})(jQuery, tpfwAvailibilityLocalizeScript, wp);
