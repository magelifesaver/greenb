(function ($) {
	...
	jQuery(document).off('tpfw_loaded_blocks').on('tpfw_loaded_blocks_timepicker_pickup', function () {
		//console.log('timepicker.js init pickup');
		init_events();
	...
	function init() {
		...
		//try {
			...
			var extra = 0;
			if (tpfwcheckoutpickup.is_rollover == 0) {
				stop = (tpfwcheckoutpickup.todayclose)[0];
				date = (tpfwcheckoutpickup.actual_date)[0];
				minTime = (tpfwcheckoutpickup.todayopen)[0];
			} else {
				if (tpfwcheckoutpickup.is_rollover in tpfwcheckoutpickup.actual_date == false) {
					throw new Error("This was an error");
				}
				stop = tpfwcheckoutpickup.is_rollover in tpfwcheckoutpickup.todayclose ? (tpfwcheckoutpickup.todayclose)[tpfwcheckoutpickup.is_rollover] : 0;
				date = tpfwcheckoutpickup.is_rollover in tpfwcheckoutpickup.actual_date ? (tpfwcheckoutpickup.actual_date)[tpfwcheckoutpickup.is_rollover] : null;
				minTime = tpfwcheckoutpickup.is_rollover in tpfwcheckoutpickup.todayopen ? (tpfwcheckoutpickup.todayopen)[tpfwcheckoutpickup.is_rollover] : 0;
			}
			var disableRange = [];
			if (minTime === 0) {
				disableRange.push(['00:00', '23:59']);
			}
			if ((tpfwcheckoutpickup.full_slots).length) {
				tpfwcheckoutpickup.full_slots.forEach( function (element, i, list) {
					if (date == element.date) {
						disableRange.push([element.time, element.time_end]);
					}
				}, this);
			}
			if ((tpfwcheckoutpickup.disable_array).length) {
				tpfwcheckoutpickup.disable_array.forEach( function (element, i, list) {
					if (date == element.date) {
			...
			var args = {
				...
				'minTime': minTime,
				'maxTime': stop,
				'step': tpfwcheckoutpickup.step,
				'timeFormat': tpfwcheckoutpickup.time_format,
				'disableTimeRanges': disableRange,
				'extra': 0,
				'showDuration': tpfwcheckoutpickup.showDuration == 1 ? true : false,
				'listWidth': 1,
				'secondsfromMidnight': parseInt(tpfwcheckoutpickup.secondsfromMidnight),

			};
			if (tpfwcheckoutpickup.show_asap == 1 && tpfwcheckoutpickup.is_open == 1 && tpfwcheckoutpickup.is_rollover == 0) {
				args.noneOption = [{
					'label': tpfwcheckoutpickup.asap_text,
					'value': tpfwcheckoutpickup.asap_text,
					'className': 'tpfw-asap'
				}, ];
			}
			$('#tpfw-time').timepicker(args);
			if (tpfwcheckoutpickup.show_asap == 1 && tpfwcheckoutpickup.is_open == 1) {
			...
			_.each($("select#tpfw-date option"), function (element, i, list) {
				...
				if (0 === tpfwcheckoutpickup.todayopen[i]) {
					$(element).prop('disabled', true);
					if (!$(element).find('span').hasClass('tpfw-date-closed')) {
						$(element).append('<span class="tpfw-date-closed"> (' + tpfwcheckoutpickup.closed_text + ')</span>');
					}
			...
			if (tpfwcheckoutpickup.preselect_time == 1 && $('#tpfw-date').find('option:selected').val() == $('#tpfw-date').find('option').first().val()) {
				$('#tpfw-time').timepicker('show');
		...
		/*} catch (err) {
			...
			let args = {
				...
				'extra': 0,
				'time_to_add': tpfwcheckoutpickup.time_to_add,
				'showDuration': tpfwcheckoutpickup.showDuration == 1 ? true : false,
				'listWidth': 1,
				'secondsfromMidnight': parseInt(tpfwcheckoutpickup.secondsfromMidnight),
			};
	...
	function update_form($this, extra, fromtimeout, time, selectedisfirst) {
		...
		if ($('#tpfw-date').find('option:selected').val() == $('#tpfw-date').find('option').first().val()) {
			...
			minTime = (tpfwcheckoutpickup.todayopen)[0];
			extra2 = extra;
		} else if (tpfwcheckoutpickup.is_rollover == 1 && $('#tpfw-date').find('option:selected').val() == $("#tpfw-date option:nth-of-type(2)").val()) {
			stop = (tpfwcheckoutpickup.todayclose)[1];
			date = (tpfwcheckoutpickup.actual_date)[1];
			minTime = (tpfwcheckoutpickup.todayopen)[1];
			extra2 = extra;
		} else {
			let index = $("option:selected", $this).index();
			stop = (tpfwcheckoutpickup.todayclose)[index];
			date = (tpfwcheckoutpickup.actual_date)[index];
			minTime = (tpfwcheckoutpickup.todayopen)[index];
			extra2 = 0;
		}
		var disableRange = [];
		if (minTime === 0) {
		...
		if ((tpfwcheckoutpickup.full_slots).length) {
			_.each(tpfwcheckoutpickup.full_slots, function (element, i, list) {
				if (date == element.date) {
					disableRange.push([element.time, element.time_end]);
				}
			}, this);
		...
		if ((tpfwcheckoutpickup.disable_array).length) {
			_.each(tpfwcheckoutpickup.disable_array, function (element, i, list) {
				if (date == element.date) {
					disableRange.push([element.start, element.stop]);
				}
			}, this);
		...
		if (fromtimeout && !selectedisfirst && time !== false) {
			...
			(!fromtimeout && tpfwcheckoutpickup.preselect_time == 1 && $('#tpfw-date').find('option:selected').val() == $('#tpfw-date').find('option').first().val())
		) {
	...
	function init_events() {
		...
		$('#tpfw-pickup-location').off('change.tpfw-location').one('change.tpfw-location', function () {
