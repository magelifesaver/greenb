	...
	public function add_instances( $instances ) {
		...
		if ( $build_instances ) {
			...
			$instances['paidmembershipspro'] = array(
				'label'             => 'Paid Memberships Pro (Billing)',
				'init'              => '#baddress1',
			...
			// Build instance data for Billing
			$fields[] = array(
				'selector' => '#pmpro_scountry',
				'data'     => '{country:short_name}',
			);
			$fields[] = array(
				'selector' => '#pmpro_saddress1',
				'data'     => '{address1:long_name}',
			);
			$fields[] = array(
				'selector' => '#pmpro_saddress2',
				'data'     => '{address2:long_name}',
			);
			$fields[] = array(
				'selector' => '#pmpro_scity',
				'data'     => '{locality:long_name}',
			);
			$fields[] = array(
				'selector' => '#pmpro_sstate',
				'data'     => '{administrative_area_level_1:short_name}',
			);
			$fields[] = array(
				'selector' => '#pmpro_szipcode',
				'data'     => '{postal_code:long_name}',
			);

			$instances['paidmembershipspro_shipping'] = array(
				'label'             => 'Paid Memberships Pro (Shipping)',
				'init'              => '#pmpro_saddress1',
				'page'              => intval( get_option( 'pmpro_checkout_page_id' ) ),
				'allowed_countries' => ( count( $pmpro_countries ) <= 5 ) ? array_keys( $pmpro_countries ) : '',
				'fields'            => $fields,
			);
