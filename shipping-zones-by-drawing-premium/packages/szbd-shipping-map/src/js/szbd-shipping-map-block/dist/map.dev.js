...
jQuery(function ($) {
  ...
  function init() {
    var szbd_checkout = {
      ...
      init: function init(loc, has_address) {
        ...
        if (szbd.is_checkout == 1) {
          ...
          if (szbd.precise_address === 'always') {
            ...
            if (szbd.auto_marker == 1 && _.isObject(location) && _.has(location, 'lat')) {
              set_marker = 'auto_marker';
            } //alert('set map');


            szbd_checkout.get_map_from_server_data(location, set_marker);
          ...
          szbd_map.init_delivery_map(); // Listen to event to update map with address data from server

          jQuery(document).off('szbd_map_update_blocks').on('szbd_map_update_blocks', function (e, latlng, has_address, loc) {
      ...
      get_map_from_server_data: function get_map_from_server_data(latlng, from_marker) {
        var loc = !_.isNull(latlng) ? szbd_checkout.from_saved_point ? latlng : latlng.extensions['szbd-shipping-map'].shipping_point : null;
        var types = [];
        var is_fail = loc == false ? true : false;

        if (!_.isNull(latlng) && (!_.isObject(loc) || !_.has(loc, 'lat') || is_fail)) {
          loc = latlng.shippingAddress;
          loc['formatted_address'] = latlng.extensions['szbd-shipping-map'].formatted_address;
          types = ['street_address'];
        }

        var is_precise_address = true;
        szbd_map.initMap(loc, is_fail, is_precise_address, types, from_marker);
      },
      is_address_empty: function is_address_empty(control_address_string) {
        try {
      ...
      try_geocode: function try_geocode(from_marker, ok_types, geo_base, s_address, comp, has_address, isPlusCode) {
        try {
          $.when(szbd_google_geocode.geocode(s_address, comp)).then(function (response) {
            ...
            if (szbd.debug == 1) {
              ...
              var debug = 'PLUSCODE REQUEST: Pluscode string: \n',
                  debug2 = s_address + '\n',
            ...
            if (status === 'OK'
            /*&& szbd_checkout.findCommonElements(results[0].types, ok_types)*/
            ) {
                if (isPlusCode) {
                  szbd_map.updatingplusCode = false;
                  szbd_map.placeMarker(results[0].geometry.location, true, false);
                  var zoom = szbd_map.get_zoom_level(results[0].types);
      ...
      test_latlng: function test_latlng(lat, lng) {
        try {
      ...
      findCommonElements: function findCommonElements(arr1, arr2) {
        return arr1.some(function (item) {
          return arr2.includes(item);
        });
      },
      ignore_szbd: function ignore_szbd(auth_error) {
    ...
    var szbd_map = {
      ...
      init_delivery_map: function init_delivery_map() {
        try {
          szbd_map.remove_marker_on_change(); // Plus Code Init
          ...
          jQuery(window).on('cma_customer_address_is_set', function (event, location) {
            szbd_map.insert_and_trigger('szbd-plus-code', 'empty'); //szbd_map.insert_and_trigger('szbd-picked', '{"lat":null}');
      ...
      initMap: function initMap(uluru, is_fail, is_precise_address, types, from_marker) {
        try {
          ...
          if (szbd.precise_address === 'at_fail') {
            if (is_fail) {
              ...
              if (!from_marker) {
                ...
                szbd_map.insert_and_trigger('szbd-plus-code', 'empty'); //szbd_map.insert_and_trigger('szbd-picked', 'empty');
          ...
          if ((szbd_map.updatingplusCode || !from_marker) && (typeof uluru == 'undefined' || uluru == null || _.isArray(uluru) || _.has(uluru, 'country'))) {
            ...
            var mapOptions = szbd_map.mapOptions;
      ...
      geocodeByArea: function geocodeByArea(uluru, mapOptions, is_precise_address, from_marker) {
        try {
      ...
      update_store_map: function update_store_map(loc, is_fail, is_precise_address, types, from_marker) {
        szbd_map.initMap(loc, is_fail, is_precise_address, types, from_marker);
      },
      geocode_plus_code_reverse: function geocode_plus_code_reverse(location) {
        try {
          $.when(szbd_google_geocode.geocode(location, null, true)).then(function (response) {
            ...
            if (szbd.debug == 1) {
              var date = new Date();
              var time = date.toLocaleTimeString() + '\n';
              var debug = 'REVERSE PLUS CODE REQUEST: ' + '\n' + '\n' + ' Address string: ' + '\n',
            ...
            if (found_pluscode != undefined) {
              szbd_map.insert_and_trigger('szbd-plus-code', found_pluscode.plus_code.compound_code);
            } else {
      ...
      geocode_plus_code: function geocode_plus_code() {
        try {
      ...
      set_map: function set_map(mapOptions, is_precise_address, uluru, from_marker) {
        var _ref, Geo, el, markerOptions;

        return regeneratorRuntime.async(function set_map$(_context) {
          while (1) {
            switch (_context.prev = _context.next) {
              ...
              case 9:
                el = document.getElementById('szbd_map');
              ...
              case 12:
                szbd_map.map = new google.maps.Map(el, mapOptions);
              ...
              case 14:
                ...
                jQuery('#szbd_map').height(400); // Insert saved shipping point from saved user meta on load
      ...
      get_zoom_level: function get_zoom_level(types) {
        try {
          var zoom = 17;
      ...
      placeMarker: function placeMarker(location) {
        ...
        return regeneratorRuntime.async(function placeMarker$(_context2) {
          while (1) {
            switch (_context2.prev = _context2.next) {
              case 0:
                from_pluscode = _args2.length > 1 && _args2[1] !== undefined ? _args2[1] : false;
                auto_marker = _args2.length > 2 && _args2[2] !== undefined ? _args2[2] : false;
                _context2.next = 4;
                return regeneratorRuntime.awrap(google.maps.importLibrary("maps"));
              ...
              case 4:
                ...
                try {
                  ...
                  store_location = szbd_map.marker.getPosition();

                  if (store_location) {
                    szbd_map.remove_marker_on_change();

                    if (!auto_marker) {
                      y = JSON.stringify(store_location); //this.insert_and_trigger('szbd-picked', y);
                      ...
                      _y = JSON.stringify(store_location); //this.insert_and_trigger('szbd-picked', y, true);

                      this.insert_and_trigger('shipping-szbd-shipping_point', _y);
                    }
      ...
      insert_and_trigger: function insert_and_trigger(el_id) //notToServer = false
      {
        ...
        try {
          ...
          console.debug([el_id, value]);

          if (el_id == 'shipping-szbd-shipping_point' && value == '') {
            return;
          }
      ...
      remove_marker_on_change: function remove_marker_on_change() {
        try {
          jQuery(document).off('input.szbd2').on('input.szbd2', '#shipping-address_1,#shipping-city,input#shipping-state', szbd_map.remove_marker);
          jQuery(document).off('change.szbd3').on('change.szbd3', '#shipping-country,select#shipping-state', szbd_map.remove_marker);
        } catch (err) {
          console.debug(err);
        }
      },
      remove_marker: function remove_marker(keep_plus_code) {
        szbd_map.do_the_remove(keep_plus_code);
      },
      do_the_remove: function do_the_remove(keep_plus_code) {
        try {
      ...
      get_country_coords: function get_country_coords(mapOptions, uluru) {
        try {
          if (szbd.countries && szbd.countries[uluru['country']]) {
            var country = szbd.countries[uluru['country']];
            mapOptions.center = {
              lat: country.lat,
              lng: country.lng
            };
      ...
      get_area_ok_types: function get_area_ok_types(comp) {
        try {
      ...
      try_second_geocode: function try_second_geocode(address, comp, mapOptions, uluru) {
        try {
          ...
          if (szbd_checkout.only_country) {
            ...
            $.when(szbd_google_geocode.geocode(address, comp)).then(function (response) {
              ...
              if (szbd.debug == 1) {
                var date = new Date();
                var debug = '2nd GEOCODE REQUEST:Address string: \n',
    ...
    var szbd_google_geocode = {
      ...
      geocode: function geocode(address, comp, isPlusCode) {
        var _ref3, Geocode, outcome, args, geocoder;

        return regeneratorRuntime.async(function geocode$(_context3) {
          while (1) {
            switch (_context3.prev = _context3.next) {
              ...
              case 2:
                ...
                return _context3.abrupt("return", outcome.promise());
