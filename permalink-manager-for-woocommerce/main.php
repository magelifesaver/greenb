...
class BeRocketLinkManager {
    ...
    public function redirect_301() {
        ...
        /*  >>> SAFETY GUARD <<<                                              *
         * Bail out if WooCommerce (and its conditional functions)            *
         * hasn't been initialised. This prevents fatal errors on pages       *
         * like wp-activate.php where WooCommerce is never loaded.            */
        if ( ! function_exists( 'is_woocommerce' ) ) {
            return;
        }
        if ( $this->get( $this->options, 'redirect_old_links' )
             && empty( $wp->query_vars['bwlm'] )
             && empty( $bwlm )
             && is_woocommerce()
             && ! is_admin()
             && ! $this->is_preview()
        ) {
            if ( function_exists( 'is_product_category' ) && is_product_category() && $this->options['category'] ) {
                global $wp_query;
                $queried_object = $wp_query->get_queried_object();
                $url            = get_term_link( $queried_object->term_id, 'product_cat' );
            }

            if ( function_exists( 'is_product_tag' ) && is_product_tag() && $this->options['tag'] ) {
                global $wp_query;
                $queried_object = $wp_query->get_queried_object();
                $url            = get_term_link( $queried_object->term_id, 'product_tag' );
            }

            if ( function_exists( 'is_product' ) && is_product() && $this->options['product'] ) {
                global $wp_query;
                $queried_object = $wp_query->get_queried_object();
