...
class RLT_Core {
    ...
    private function __construct() {
        ...
        // Expose AJAX actions for debugging. These will allow administrators to test
        // their API key and recompute lead‑time values from the admin UI. The
        // actions are only available when current user can manage WooCommerce.
        if (is_admin()) {
            add_action('wp_ajax_rlt_test_api_key', array($this, 'ajax_test_api_key'));
            add_action('wp_ajax_rlt_get_debug_info', array($this, 'ajax_get_debug_info'));
        }
    }
    ...
    /**
     * Public wrapper around the private travel time lookup. This method is
     * provided for debugging purposes so that the settings page can expose
     * the computed travel time to administrators. Returns the travel time
     * in minutes or false when unavailable.
     *
     * @return int|false
     */
    public function get_current_travel_time() {
        return $this->get_travel_time();
    }

    /**
     * Public wrapper for computing the earliest slot timestamp. Allows
     * external callers (debug page) to calculate the earliest slot based on a
     * given lead time. Accepts minutes and returns a UNIX timestamp or false
     * if no valid slot exists.
     *
     * @param int    $lead_minutes Minutes of total lead time.
     * @param string $mode         Mode string: 'delivery' or 'pickup'.
     *
     * @return int|false
     */
    public function get_earliest_slot_timestamp($lead_minutes, $mode = 'delivery') {
        return $this->compute_earliest_slot_timestamp($lead_minutes, $mode);
    }

    /**
     * Tests an arbitrary API key by attempting a simple request against the
     * Google Distance Matrix API. Uses two fixed addresses to minimise
     * variables. Returns true when the API returns a valid response, or
     * false when the key appears invalid or the request fails.
     *
     * @param string $key The API key to test.
     * @return bool
     */
    public static function test_api_key($key) {
        $key = trim($key);
        if ($key === '') {
            return false;
        }
        // Use two known addresses that should always return a result.
        $origin      = rawurlencode('New York, NY');
        $destination = rawurlencode('Los Angeles, CA');
        $url         = 'https://maps.googleapis.com/maps/api/distancematrix/json?units=imperial&origins=' . $origin . '&destinations=' . $destination . '&key=' . rawurlencode($key);
        $response    = wp_remote_get($url, array('timeout' => 10));
        if (is_wp_error($response)) {
            return false;
        }
        ...
        $data = json_decode($body, true);
        if (isset($data['status']) && $data['status'] === 'OK' && isset($data['rows'][0]['elements'][0]['status'])) {
            $element_status = $data['rows'][0]['elements'][0]['status'];
            return ($element_status === 'OK');
        }
        return false;
    }

    /**
     * Aggregates a set of diagnostic values for debugging purposes. The
     * returned array contains the plugin configuration, computed travel
     * time, total lead time and the earliest slot timestamp. This is
     * intended for display in the admin debug page.
     *
     * @return array
     */
    public function get_debug_info() {
        ...
        $prep    = isset($options['prep_time']) ? absint($options['prep_time']) : 0;
        $dispatch = isset($options['dispatch_buffer']) ? absint($options['dispatch_buffer']) : 0;
        // Attempt to compute live travel time. If unavailable, record false.
        $travel = $this->get_travel_time();
        $default_travel = isset($options['default_travel_time']) ? absint($options['default_travel_time']) : 0;
        $travel_used = ($travel !== false && $travel >= 0) ? $travel : $default_travel;
        $total_minutes = $prep + $dispatch + $travel_used;
        $earliest_ts   = $this->compute_earliest_slot_timestamp($total_minutes);
        $slot_time     = false;
        $slot_string   = '';
        $slot_end      = '';
        if ($earliest_ts) {
            $slot_time   = $earliest_ts;
            $slot_length = isset($options['slot_length']) ? absint($options['slot_length']) : 60;
            $slot_end_ts = $earliest_ts + ($slot_length * 60);
            $format      = get_option('date_format') . ' ' . get_option('time_format');
            $slot_string = wp_date($format, $earliest_ts);
            $slot_end    = wp_date($format, $slot_end_ts);
        }
        return array(
            'settings' => $options,
            'prep_time' => $prep,
            'dispatch_buffer' => $dispatch,
            'live_travel_time' => $travel,
            'default_travel_time' => $default_travel,
            'travel_time_used' => $travel_used,
            'total_lead_time' => $total_minutes,
            'earliest_slot_timestamp' => $slot_time,
            'earliest_slot_string' => $slot_string,
            'earliest_slot_end' => $slot_end,
        );
    }

    /**
     * AJAX callback to test a provided API key. Expects a POST param
     * "api_key". Returns JSON {valid: true|false}. Only administrators
     * can perform this test.
     */
    public function ajax_test_api_key() {
        if (!current_user_can('manage_woocommerce')) {
            wp_send_json_error(array('message' => __('You do not have permission to perform this action.', 'rlt')));
        }
        // Verify nonce to protect against CSRF.
        check_ajax_referer('rlt_debug_nonce', '_ajax_nonce');
        $key    = isset($_POST['api_key']) ? sanitize_text_field(wp_unslash($_POST['api_key'])) : '';
        $result = self::test_api_key($key);
        wp_send_json_success(array('valid' => $result));
    }

    /**
     * AJAX callback to return debug info. Returns the same array as
     * get_debug_info() in JSON. Only administrators can access.
     */
