...
class Base_counter extends Base_simple implements Interface_required
{
     ...
     * @var string
     */
    protected $type = 'acf_base_counter';
    ...
    {
        $fields = [
            'label'                => __('%s', 'publishpress-checklists-pro'),
            'label_settings'       => __('Number of characters in %s field', 'publishpress-checklists-pro'),
            'label_min_singular'   => __('Minimum of %%s character in %s field', 'publishpress-checklists-pro'),
            'label_min_plural'     => __('Minimum of %%s characters in %s field', 'publishpress-checklists-pro'),
            'label_max_singular'   => __('Maximum of %%s character in %s field', 'publishpress-checklists-pro'),
            'label_max_plural'     => __('Maximum of %%s characters in %s field', 'publishpress-checklists-pro'),
            'label_exact_singular' => __('%%s character in %s field', 'publishpress-checklists-pro'),
            'label_exact_plural'   => __('%%s characters in %s field', 'publishpress-checklists-pro'),
            'label_between'        => __('Between %%s and %%s characters in %s field', 'publishpress-checklists-pro'),
        ];

        foreach ($fields as $key => $value) {
            $this->lang[$key] = sprintf($value, $this->label);
        }
    }
    ...
    public function get_setting_field_html($css_class = '')
    {
        ...
        if ($this->maxLength) {
            // Get the field markup for max value
            $max_field = sprintf(
                '<input type="hidden" name="%s" value="%s"><input type="text" id="%s" value="%s" class="pp-checklists-small-input pp-checklists-number" disabled /><span class="pp-checklists-field-description">%s</span>',
                "{$this->module->options_group_name}[{$option_name_max}][{$post_type}]",
                $this->maxLength,
                "{$post_type}-{$this->module->slug}-{$option_name_max}",
                $this->maxLength,
                esc_html__('Got value from ACF Configuration.', 'publishpress-checklists-pro')
            );
        }
