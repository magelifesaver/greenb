...
class AAA_OC_Ajax_Payment_Update {
	...
	// Resolve transaction IDs (core + your gateway)
	$wc_txn = isset($order) && $order ? $order->get_transaction_id() : '';
	if ( ! $wc_txn && isset($order) && $order ) { $wc_txn = (string) $order->get_meta( '_transaction_id', true ); }
	$wc_txn = is_string($wc_txn) && $wc_txn !== '' ? trim($wc_txn) : null;

	$gw_txn = (string) get_post_meta( $order_id, 'aaa_oc_gateway_ref', true );
	$gw_txn = $gw_txn !== '' ? trim($gw_txn) : null;
