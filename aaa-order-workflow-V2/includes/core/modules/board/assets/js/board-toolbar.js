/**
 * File: /wp-content/plugins/aaa-order-workflow/assets/js/board-toolbar.js
 * Purpose: Toolbar shell + sheets + empty sidebar. NO feed logic here.
 * Version: 1.4.2
 */
;(function($){
  'use strict';
  ...
  const LSFP = 'aaaOC_fullpage';

  // ---------- CSS (kept: header/sheets/sidebar baseline) ----------
  function injectCss(){ if($('#aaa-oc-shell-style').length) return;
    const css = `
      ...
      .aaa-oc-sheet .aaa-oc-body{max-height:160px;overflow:auto;padding:10px 12px}

      /* Right sidebar shell (no feed logic here) */
  ...
  // ---------- Panels + sidebar shell (no feed logic) ----------
  function buildPanels(){
    ...
    // Sidebar shell is ready—announce so feed module can mount
    $(document).trigger('aaa-oc:sidebar:ready');
  }
  ...
  function getAdminBarHeight(){
    ...
    return Math.max(0, bar.offsetHeight || 0);
  }
  ...
  // ---------- Full-page ----------
  function applyFP(){
    document.documentElement.classList.toggle('aaa-oc-fullpage', localStorage.getItem(LSFP)==='1');
    setSheetTop();
  }

  // ---------- Events (no feed handlers here) ----------
  function bind(){
    ...
    $(document).on('click','.aaa-oc-toggle-fullpage',function(){
      const on = localStorage.getItem(LSFP)==='1'; localStorage.setItem(LSFP, on?'0':'1'); applyFP();
    });
  ...
  $(function(){
    ...
    setSheetTop();
    // Signal toolbar readiness for filters/actions binders
    $(document).trigger('aaa-oc:toolbar:ready');
    log('header + sheets + sidebar shell ready');
  });
