...
final class V7000 extends Update
{
    ...
    private function update_file_storage(): void
    {
        ...
        foreach ($this->file_repository->find_all() as $file_path => $data) {
            $this->update_file_core_changes($file_path, $data);
        }
        foreach ($this->file_repository->find_all() as $file_path => $data) {
            $this->update_file_metabox_changes($file_path, $data);
        }
        foreach ($this->file_repository->find_all() as $file_path => $data) {
            $this->update_file_jetengine_changes($file_path, $data);
        }
    }

    private function update_file_jetengine_changes(string $file_path, array $data): void
    {
    ...
    private function update_file_metabox_changes(string $file_path, array $data): void
    {
        $type = $data['list_screen']['type'] ?? '';
    ...
    private function update_file_core_changes(string $file_path, array $data): void
    {
