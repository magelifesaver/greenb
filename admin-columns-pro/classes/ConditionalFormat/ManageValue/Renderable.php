...
class Renderable implements CellRenderer
{

    private Operators $operators;

    private ConditionalFormat\Entity\Rules $rules;
    ...
    public function __construct(
        ...
        Operators $operators,
        ConditionalFormat\Entity\Rules $rules,
    ...
    {
        $value = $this->renderable->render_cell($column_id, $row_id);
        ...
        if (ProcessFormatters::DEFAULT === $value) {
            return $value;
        }
        ...
        foreach ($this->rules->get_rule_collection() as $rule) {
            if ($rule->get_column_name() !== (string)$column->get_id()) {
                continue;
            }
