...
class AuthorName extends ACP\Column\AdvancedColumnFactory
{
    ...
    protected function get_formatters(Config $config): FormatterCollection
    {
        return parent::get_formatters($config)
                     ->prepend(new AC\Value\Formatter\Post\Author());
    }
    ...
    protected function get_export(Config $config): ?ACP\Export\Service
    {
        if ($config->get('display_author_as') === UserProperty::PROPERTY_GRAVATAR) {
            return new ACP\Export\Model\FormatterCollection(new FormatterCollection([
                new AC\Value\Formatter\Post\Author(),
                new ACP\Value\Formatter\User\GravatarUrl(),
            ]));
        }

        $formatters = $this->user_factory->create($config)->get_formatters();
        $formatters->prepend(new AC\Value\Formatter\Post\Author());
        $formatters->add(new AC\Value\Formatter\StripNewLine());
