...
class ChildPages extends ACP\Column\AdvancedColumnFactory
{
    ...
    protected function get_formatters(Config $config): FormatterCollection
    {
        return new FormatterCollection([
            new ACP\Value\Formatter\Post\ChildIds([(string)$this->post_type]),
            new Formatter\Post\PostTitle(),
            new Formatter\Post\PostLink('edit_post'),
            Formatter\Collection\Separator::create_from_config($config),
        ]);
    }

    protected function get_export(Config $config): ?ACP\Export\Service
    {
        return new Export\Model\Post\ChildPages((string)$this->post_type);
    }
