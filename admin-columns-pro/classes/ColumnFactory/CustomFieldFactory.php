...
class CustomFieldFactory extends ACP\Column\AdvancedColumnFactory
{
    ...
    protected function get_settings(Config $config): ComponentCollection
    {
        ...
        if ($config->get('field_type') === FieldType::TYPE_POST) {
    ...
    protected function get_feature_settings_builder(Config $config): FeatureSettingBuilder
    {
        ...
        $edit_setting = $config->get('field_type') === FieldType::TYPE_TEXT
        ...
        if ($config->get('field_type') === FieldType::TYPE_DATE) {
    ...
    protected function get_editing(Config $config): ?Service
    {
        $storage = new Editing\Storage\Meta(
            (string)$config->get('field'), $this->table_screen_context->get_meta_type()
        );
    ...
    {
        $meta_key = (string)$config->get('field', '');
        ...
        switch ((string)$config->get('field_type')) {
            ...
            case FieldType::TYPE_BOOLEAN:
                return $this->sorting_factory->create(
                    $this->table_screen_context->get_meta_type(),
                    $meta_key,
            ...
            case FieldType::TYPE_SELECT :
                return $config->get('is_multiple', 'on') === 'on'
                    ? null
                    : $this->sorting_factory->create($this->table_screen_context->get_meta_type(), $meta_key);
            ...
            case FieldType::TYPE_COLOR :
            default :
                return $this->sorting_factory->create($this->table_screen_context->get_meta_type(), $meta_key);
        }
    }
    ...
    protected function get_search(Config $config): ?Search\Comparison
    {
        ...
        switch ($config->get('field_type')) {
            case FieldType::TYPE_ARRAY:
                return new Search\Comparison\Meta\Serialized($meta_key);
            case FieldType::TYPE_BOOLEAN:
                return new Search\Comparison\Meta\Checkmark($meta_key);
            case FieldType::TYPE_DATE:
                return Search\Comparison\Meta\DateFactory::create(
                    $this->get_date_save_format($config),
                    $meta_key,
                    $this->create_query($meta_key)
                );
            case FieldType::TYPE_IMAGE:
            case FieldType::TYPE_MEDIA:
                return new Search\Comparison\Meta\Media($meta_key, $this->create_query($meta_key));
            case FieldType::TYPE_SELECT:
                $options = $this->get_select_options($config);

                return $config->get('is_multiple', 'on') === 'on'
                    ? new Search\Comparison\Meta\MultiSelect($meta_key, $options)
                    : new Search\Comparison\Meta\Select($meta_key, $options);

            case FieldType::TYPE_NUMERIC:
                return new Search\Comparison\Meta\Number($meta_key);
            case FieldType::TYPE_POST:
                return $config->get('is_multiple', 'on') === 'on'
            ...
            case FieldType::TYPE_USER:
                return new Search\Comparison\Meta\User($meta_key);
            case FieldType::TYPE_COUNT:
                return null;
            case FieldType::TYPE_NON_EMPTY:
                return new Search\Comparison\Meta\EmptyNotEmpty($meta_key);
            case FieldType::TYPE_URL:
            case FieldType::TYPE_TEXT:
            case FieldType::TYPE_COLOR:
            default:
                return new Search\Comparison\Meta\SearchableText($meta_key, $this->create_query($meta_key));
        }
    }

    protected function get_export(Config $config): ?Export\Service
    {
        $meta_storage = new AC\Storage\MetaData($this->table_screen_context->get_meta_type());
        $meta_key = $config->get('field', '');

        switch ((string)$config->get('field_type', '')) {
            case FieldType::TYPE_POST :
            case FieldType::TYPE_USER :
                return new Export\Model\FormatterCollection(
                    $this->get_formatters($config)->with_formatter(new Formatter\StripNewLine())
                );
            case FieldType::TYPE_ARRAY :
            case FieldType::TYPE_COUNT :
                return new Export\Model\FormatterCollection($this->get_formatters($config));
            case FieldType::TYPE_NON_EMPTY :
                return new Export\Model\CustomField\HasContent($meta_storage, $meta_key);
            case FieldType::TYPE_DATE :
                return new Export\Model\CustomField\Date($meta_storage, $meta_key);
            case FieldType::TYPE_IMAGE :
            case FieldType::TYPE_MEDIA :
                return new Export\Model\CustomField\Image($meta_storage, $meta_key);
            case FieldType::TYPE_BOOLEAN :
            case FieldType::TYPE_COLOR :
            case FieldType::TYPE_TEXT :
            case FieldType::TYPE_URL :
            ...
            default :
                return new Export\Model\Formatter(
                    new Formatter\Meta($this->table_screen_context->get_meta_type(), $meta_key)
                );
        }
    }
                ...
                return new ConditionalFormat\FormattableConfig(
                    new ConditionalFormat\Formatter\DateFormatter\DateValueFormatter(
                        new FormatterCollection([
                            new Formatter\Meta($this->table_screen_context->get_meta_type(), $config->get('field', '')),
                            new Formatter\Date\DateMapper($this->get_date_save_format($config), 'Y-m-d'),
                        ])
                    )
                );
            case FieldType::TYPE_ARRAY :
    ...
    {
        return (new StoredDateFormat(new Context($config)))->apply_filters(
    ...
    {
        switch (true) {
            case $this->table_screen_context->has_post_type():
                return (new Meta\QueryMetaFactory())->create_with_post_type(
                    $meta_key,
                    (string)$this->table_screen_context->get_post_type()
                );
            default:
                return (new Meta\QueryMetaFactory())->create($meta_key, $this->table_screen_context->get_meta_type());
        }
    }
}