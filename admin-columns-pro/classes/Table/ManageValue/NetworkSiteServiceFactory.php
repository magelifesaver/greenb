<?php

declare(strict_types=1);

namespace ACP\Table\ManageValue;

use AC;
use AC\TableScreen\ManageValueService;
use AC\TableScreen\ManageValueServiceFactory;
use ACP\TableScreen;
use InvalidArgumentException;

class NetworkSiteServiceFactory implements ManageValueServiceFactory
{

    public function can_create(AC\TableScreen $table_screen): bool
    {
        return $table_screen instanceof TableScreen\NetworkSite;
    }

    public function create(
        AC\TableScreen $table_screen,
        AC\CellRenderer $renderable,
        int $priority = 100
    ): ManageValueService {
        if ( ! $table_screen instanceof TableScreen\NetworkSite) {
            throw new InvalidArgumentException('Invalid table screen');
        }

        return new NetworkSite($renderable, $priority);
    }

}