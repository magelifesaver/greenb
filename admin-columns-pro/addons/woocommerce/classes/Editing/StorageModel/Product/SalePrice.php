...
class SalePrice extends Price
{
    ...
    {
        if ('clear' === $this->value->get_price_type() || ('flat' === $this->value->get_price_type() && in_array(
                    $this->value->get_price(),
                    ['', '0'],
                    true
                ))) {
        ...
        if ($price >= (float)$this->product->get_regular_price()) {
            return new WP_Error(
                'invalid-price',
                __('Sale price can not be higher than the regular price.', 'codepress-admin-columns')
            );
        }
    ...
    private function remove_sale_price(): int
    {
    ...
    public function get_price(): float
    {
        return $this->value->is_price_based_on_regular()
            ? (float)$this->product->get_regular_price()
            : (float)$this->product->get_sale_price();
    }
