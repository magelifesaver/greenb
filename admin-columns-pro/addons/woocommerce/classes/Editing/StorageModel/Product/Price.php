...
class Price
{
    ...
    protected function round_price(float $price): float
    {
    ...
    protected function get_calculated_price(): float
    {
        switch ($this->value->get_price_type()) {
            case 'increase_percentage':
                return $this->increase_price_by_percentage($this->get_price(), $this->value->get_percentage());

            case 'decrease_percentage':
                return $this->decrease_price_by_percentage($this->get_price(), $this->value->get_percentage());
    ...
    protected function increase_price_by_percentage(float $price, float $percentage): float
    {
        return $price + (($price / 100) * $percentage);
    }

    protected function decrease_price_by_percentage(float $price, float $percentage): float
    {
        return $price - (($price / 100) * $percentage);
    }

    protected function get_price(): float
    {
