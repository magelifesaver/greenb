...
class FileFactory extends FieldFactory
{
    ...
    protected function get_export(Config $config): ?ACP\Export\Service
    {
        return new ACP\Export\Model\FormatterCollection(
            $this->get_base_formatters($config)
                 ->with_formatter(new AC\Value\Formatter\Media\AttachmentUrl())
        );
    }

    protected function get_search(Config $config): ?ACP\Search\Comparison
    {
        return new ACP\Search\Comparison\Meta\Media($this->field->get_name(), $this->get_query_meta());
    }
