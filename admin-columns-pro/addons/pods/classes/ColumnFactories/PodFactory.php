...
class PodFactory extends AC\ColumnFactories\BaseFactory
{
    ...
    protected function get_factories(TableScreen $table_screen): ColumnFactoryDefinitionCollection
    {
        ...
        switch (true) {
            case $table_screen instanceof AC\TableScreen\Post :
                return $this->get_factories_by_pod_name((string)$table_screen->get_post_type());

            case $table_screen instanceof AC\TableScreen\Media :
                return $this->get_factories_by_pod_name('media');

            case $table_screen instanceof AC\TableScreen\User :
                return $this->get_factories_by_pod_name('user');

            case $table_screen instanceof AC\TableScreen\Comment :
                return $this->get_factories_by_pod_name('comment');

            case $table_screen instanceof ACP\TableScreen\Taxonomy :
                return $this->get_factories_by_pod_name((string)$table_screen->get_taxonomy());
    ...
    protected function get_factories_by_pod_name($pod_name): ColumnFactoryDefinitionCollection
    {
            ...
            if (array_key_exists($field->get_type(), $mapping)) {
                $collection->add(
                    new AC\Type\ColumnFactoryDefinition(
                        $mapping[$field->get_type()], $arguments
                    )
                );
            }
        }
