...
class Vector_Sync_Api_Client {
    ...
    public function list_vector_spaces( $service ) {
        if ( 'openai' === $service ) {
            ...
            // requests and the assistants beta header【799506338848902†L148-L156】.  Where
            // provided, also append organisation and project IDs.  Users
            // belonging to multiple organisations or projects may need these
            // headers for the API to return all stores.  Omitting them
            // defaults to the account’s primary organisation and project.
            $headers = array(
            ...
            // Attempt to extract a human‑readable error message from the
            // response body.  OpenAI errors are returned in an `error` array
            // with a `message` field.  Fallback to a generic string if
            // unavailable.
            $err_msg = __( 'Unable to list OpenAI vector stores.', 'vector-sync' );
            if ( is_array( $body ) && isset( $body['error']['message'] ) && is_string( $body['error']['message'] ) ) {
                $err_msg .= ' ' . $body['error']['message'];
            }
            return new WP_Error( 'api_error', $err_msg, $body );
        }
        if ( 'pinecone' === $service ) {
