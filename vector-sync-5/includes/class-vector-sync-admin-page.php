...
class Vector_Sync_Admin_Page {
    ...
    private function render_api_tab( array $settings ) {
        ...
        // Verification buttons.  Users can test connectivity for each
        // service directly from the API settings page.  The AJAX handler
        // checks whether the provided API key (and environment where
        // applicable) can successfully list vector spaces.
        echo '<tr><th scope="row">' . esc_html__( 'Verify Pinecone', 'vector-sync' ) . '</th><td>';
        echo '<button type="button" class="button" id="pinecone-verify-api">' . esc_html__( 'Verify Credentials', 'vector-sync' ) . '</button> ';
        echo '<span class="vector-sync-verify-status" id="pinecone-status"></span>';
        echo '<p class="description">' . esc_html__( 'Click to test your Pinecone API key and environment.', 'vector-sync' ) . '</p>';
        echo '</td></tr>';
        echo '<tr><th scope="row">' . esc_html__( 'Verify OpenAI', 'vector-sync' ) . '</th><td>';
        echo '<button type="button" class="button" id="openai-verify-api">' . esc_html__( 'Verify Credentials', 'vector-sync' ) . '</button> ';
        echo '<span class="vector-sync-verify-status" id="openai-status"></span>';
        echo '<p class="description">' . esc_html__( 'Click to test your OpenAI API key.', 'vector-sync' ) . '</p>';
        echo '</td></tr>';
        echo '</table>';
        ?>
        <script type="text/javascript">
        jQuery(document).ready(function($){
            function verifyService(service) {
                var btn = $('#' + service + '-verify-api');
                var status = $('#' + service + '-status');
                status.text('<?php echo esc_js( __( 'Verifying...', 'vector-sync' ) ); ?>').css('color','');
                $.post(ajaxurl, {
                    action: 'vector_sync_verify_api',
                    service: service.replace('-verify-api',''),
            ...
            $('#pinecone-verify-api').on('click', function(){ verifyService('pinecone-verify-api'); });
            $('#openai-verify-api').on('click', function(){ verifyService('openai-verify-api'); });
        });
        </script>
        <?php
    }
