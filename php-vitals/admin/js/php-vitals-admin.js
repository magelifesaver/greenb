(function($) {
    ...
    function getGrade(time) {
        return new Promise((resolve, reject) => {
            ...
            fetch(phpvitals.ajaxurl, {
                ...
                .then(data => {
                    try {
                        console.log('getGrade response:', data);
    ...
    function processBenchmarkResults() {
        try {
            // First, add the totals row to the table
            addTotalsRow();

            // Then get the grade and process everything
            getGrade(totalTime).then(grade => {
                try {
                    ...
                    // Finally, save the results with a longer delay to avoid conflicts
                    setTimeout(() => {
                        try {
    ...
    function saveBenchmarkResults(grade) {
        try {
            ...
            // Don't let this crash the benchmark process
        }
    }
            ...
            .then(response => {
                if (response.success) {
                    ...
                    if (testIndex < data.total_tests - 1) {
                        ...
                        // Complete the benchmark process in sequence
                        isRunning = false;
                        $('#runBenchmark').prop('disabled', false);
                        jQuery('#loading').hide();
