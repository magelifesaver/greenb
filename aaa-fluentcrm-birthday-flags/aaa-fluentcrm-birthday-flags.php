...
register_activation_hook( __FILE__, function () {
    ...
    // Initialise run time option if missing. A blank value disables scheduling.
    if ( false === get_option( AAA_FCRM_BDAY_OPT_RUNTIME, false ) ) {
        update_option( AAA_FCRM_BDAY_OPT_RUNTIME, '' );
    }
    // Unschedule any previously scheduled events so the plugin does not auto-run after activation.
    if ( $ts = wp_next_scheduled( AAA_FCRM_BDAY_CRON ) ) {
        wp_unschedule_event( $ts, AAA_FCRM_BDAY_CRON );
    }
});

// On deactivation, clear the scheduled cron job, if any.
...
---------------------------------------------------------------------------*/
// Add submenu entry under FluentCRM in the WordPress admin.
 ...
 * @param array  $selected_slugs Selected tag slugs
 * @param array  $all_tags       All tags available for selection (each with 'title' and 'slug')
 */
...
/**
 * Settings page callback. Presents form inputs for configuring the birthday flagger.
 */
function aaa_fcrm_bday_settings_page() {
    ...
    // Load all FluentCRM tags for selection. If FluentCRM isn't active, we'll display an error.
    $all_tags = array();
    if ( function_exists( 'FluentCrmApi' ) ) {
        try {
            ...
            foreach ( $rows as $row ) {
                // $row may be an array or an object, depending on FluentCRM version.
                $all_tags[] = array(
    ...
    if ( isset( $_POST['aaa_fcrm_bday_save'] ) && check_admin_referer( 'aaa_fcrm_bday_save' ) ) {
        ...
        // Save and validate run time. Accept HH:MM (24-hour) or blank. Anything else disables scheduling.
        $run_time_in = isset( $_POST['run_time'] ) ? sanitize_text_field( $_POST['run_time'] ) : '';
        $run_time_in = trim( $run_time_in );
        if ( '' !== $run_time_in && ! preg_match( '/^([01]?\d|2[0-3]):([0-5]\d)$/', $run_time_in ) ) {
            // Invalid format; disable scheduling
            $run_time_in = '';
        }
        ...
        // If a runtime is set, schedule the cron for the next occurrence at that time in the site's timezone
        if ( '' !== $run_time_in ) {
            try {
                $tz  = wp_timezone();
                $now = new DateTimeImmutable( 'now', $tz );
                list( $h, $m ) = array_map( 'intval', explode( ':', $run_time_in ) );
                // Build a candidate time today at the chosen hour:minute
                $candidate = new DateTimeImmutable( $now->format( 'Y-m-d' ) . sprintf( ' %02d:%02d:00', $h, $m ), $tz );
                // If the candidate has already passed, move to tomorrow
                if ( $candidate <= $now ) {
                    $candidate = $candidate->modify( '+1 day' );
                }
                $ts = $candidate->getTimestamp();
            } catch ( \Throwable $e ) {
                // Fallback: schedule tomorrow at the desired time
    ...
    <div class="wrap">
        ...
        <?php
        // Show next scheduled run or inform if disabled
...
/**
 * Cron + Manual runner.
 * Loops through contacts and applies / removes birthday tags based on the configured window.
 */
function aaa_fcrm_bday_run() {
    // Bail if FluentCRM is missing
    if ( ! function_exists( 'FluentCrmApi' ) ) {
        if ( AAA_FCRM_BDAY_DEBUG ) {
...
---------------------------------------------------------------------------*/
// Add a direct "Settings" link to the plugin row in the Plugins list page. This makes it easy for users to find the settings page.
