...
class WF_SFWF_Forecast_Index {
    ...
    /**
     * Retrieves all rows from the custom product forecast index table.  This
     * allows the forecast grid to read a fully denormalised snapshot of
     * forecast data without issuing dozens of meta queries per product.
     *
     * Note: This method queries the table defined by
     * AAA_OC_ProductForecast_Table_Installer, not the legacy flag table.  It
     * returns an array of associative arrays keyed by column names.
     *
     * @return array[] List of rows from the forecast index table.
     */
    public static function get_all_rows() {
        global $wpdb;
        // Use the helpers to get the correct table name when available.
        $table = null;
        if ( class_exists( 'AAA_OC_ProductForecast_Helpers' ) ) {
            $table = AAA_OC_ProductForecast_Helpers::table_index();
        } else {
            // Fallback to the new table name with prefix.
            $table = $wpdb->prefix . 'aaa_oc_productforecast_index';
        }
