    ...
    if(loadBtn && rows){
      on(loadBtn, 'click', function(){
        if(!roleSel || !roleSel.value){
          rows.innerHTML = '<tr><td colspan="8" style="text-align:center;">No role selected.</td></tr>'; return;
        }
        ...
        fetch(url, {credentials:'same-origin'}).then(r=>r.json()).then(function(res){
          if(res && res.success){ rows.innerHTML = res.data.rows || '<tr><td colspan="8" style="text-align:center;color:#777;">No users.</td></tr>'; }
          else { rows.innerHTML = '<tr><td colspan="8">Failed to load.</td></tr>'; }
        }).catch(function(){ rows.innerHTML = '<tr><td colspan="8">Error.</td></tr>'; });
      });
