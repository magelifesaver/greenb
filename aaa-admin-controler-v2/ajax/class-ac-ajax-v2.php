 ...
 *
 * This updated version adds support for an `online_only` parameter on the
 * sessions loader. When passed as a truthy value the handler skips users
 * who are not currently online, reducing overhead when inspecting roles
 * with a large number of accounts (e.g. customers). All other logic
 * remains unchanged.
 */
...
class AC_Ajax_V2 {
        ...
        public static function load_sessions(){
                ...
                // Determine if we should only return users who are currently online.
                $online_only = false;
                if ( isset($_GET['online_only']) ){
                        $flag = $_GET['online_only'];
                        // Accept values like 1, '1', 'true', true
                        $online_only = filter_var( $flag, FILTER_VALIDATE_BOOLEAN );
                }
                ...
                foreach( $users as $u ){
                        ...
                        // Skip offline users when the filter is enabled
