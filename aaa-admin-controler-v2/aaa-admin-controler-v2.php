...
function aaa_ac_get_all_roles(){
	$editable = get_editable_roles(); $out = [];
...
function aaa_ac_is_user_online( $user_id ){
	$tokens = WP_Session_Tokens::get_instance( $user_id )->get_all(); $now=time();
	foreach( $tokens as $t ){ if( !empty($t['expiration']) && (int)$t['expiration']>$now ) return true; }
	return false;
}
function aaa_ac_latest_session_login( $user_id ){
	$tokens = WP_Session_Tokens::get_instance( $user_id )->get_all(); $login=0;
	foreach( $tokens as $t ){ if( !empty($t['login']) && (int)$t['login']>$login ) $login=(int)$t['login']; }
...
add_action('network_admin_edit_aaa_ac_save_roles', function(){
	...
	wp_redirect( add_query_arg(['page'=>'aaa-ac-online','updated'=>'1','tab'=>'settings'], network_admin_url('admin.php')) ); exit;
});
...
add_action('network_admin_edit_aaa_ac_save_cron', function(){
	...
	wp_redirect( add_query_arg(['page'=>'aaa-ac-online','tab'=>'settings','cronupdated'=>'1'], network_admin_url('admin.php')) ); exit;
});
...
add_action('admin_init', function(){
	...
	wp_safe_redirect( add_query_arg(['page'=>'aaa-ac-online','tab'=>'sessions','ended'=>'1'], network_admin_url('admin.php')) ); exit;
});
