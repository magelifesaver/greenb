...
final class AAA_BSS_Shortcode {
    ...
    private function render_product_rows( array $product_ids, $group_key, $max_total ) {
        ...
        foreach ( $product_ids as $pid ) {
            ...
            if ( ! $product ) {
                ...
                $html .= '<div class="aaa-bss-name">' . esc_html__( 'Missing/invalid product ID:', 'aaa-bss' ) . ' ' . esc_html( (int) $pid ) . '</div>';
    ...
    public function ajax_add_to_cart() {
        ...
        foreach ( $step2 as $pid => $qty ) {
            $cart_item_data = [];
            if ( $step2_is_free ) { $cart_item_data['aaa_bss_free'] = true; }
        ...
        wp_send_json_success( [ 'message' => 'Added to cart.', 'redirectUrl' => $redirect ] );
    }
