...
// 2) Enqueue sorting CSS/JS on the report page
add_action( 'admin_enqueue_scripts', 'aaa_enqueue_report_assets' );
function aaa_enqueue_report_assets( $hook_suffix ) {
    $ok = in_array( $hook_suffix, [
        'toplevel_page_aaa-daily-report-v3',
        'woocommerce_page_aaa-daily-report-v3',
    ], true );
    if ( ! $ok ) return;
...
// 3) Register hidden submenu pages (so admin.php?page=... works)
add_action( 'admin_menu', function() {
    ...
    // 4) Hide from classic WooCommerce submenu (no duplicates)
    remove_submenu_page( 'woocommerce', 'aaa-daily-report-v3' );
    remove_submenu_page( 'woocommerce', 'aaa-report-email-settings' );
}, 9);

// 5) Add menu items under Analytics (visible links)
add_filter( 'woocommerce_analytics_report_menu_items', function( $pages ) {
    $pages[] = [
        'id'     => 'aaa-daily-report',
        'title'  => __( 'Daily Report (v3)', 'aaa-daily-reporting' ),
        'parent' => 'woocommerce-analytics',
        'path'   => '/analytics/aaa-daily-report',
    ];
    $pages[] = [
        'id'     => 'aaa-email-settings',
        'title'  => __( 'Email Settings', 'aaa-daily-reporting' ),
        'parent' => 'woocommerce-analytics',
        'path'   => '/analytics/aaa-email-settings',
    ];
    return $pages;
});

// 6) Redirect Analytics SPA paths → PHP pages
add_action( 'admin_init', function() {
    if ( isset( $_GET['page'], $_GET['path'] ) && $_GET['page'] === 'wc-admin' ) {
        if ( $_GET['path'] === '/analytics/aaa-daily-report' ) {
            wp_safe_redirect( admin_url( 'admin.php?page=aaa-daily-report-v3' ) );
            exit;
        }
        if ( $_GET['path'] === '/analytics/aaa-email-settings' ) {
            wp_safe_redirect( admin_url( 'admin.php?page=aaa-report-email-settings' ) );
            exit;
        }
    }
});

// 7) Enqueue wc-admin bridge JS (registers SPA routes so highlight works)
add_action( 'admin_enqueue_scripts', function() {
    ...
    wp_register_script(
        ...
        plugins_url( 'assets/js/aaa-analytics-bridge.js', __FILE__ ),
        [ 'wc-components', 'wp-element', 'wp-i18n', 'wp-polyfill' ],
        '1.0.0',
        true
    );
    wp_enqueue_script( 'aaa-analytics-bridge' );
});
...
echo '<div class="wrap"><h1 style="display:flex;justify-content:space-between;align-items:center;">'
   . '<span>AAA Daily Report (v3.1)</span>'
   . '<a href="' . esc_url( admin_url( 'admin.php?page=aaa-report-email-settings' ) ) . '" target="_blank" class="button button-secondary">Email Settings ⚙️</a>'
   . '</h1>';
    echo '<style>
