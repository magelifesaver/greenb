...
class WC_Account_Funds_Email_Account_Funds_Increase extends WC_Email {
	...
	public function __construct() {
		...
		$funds_name = \Kestrel\Account_Funds\Settings\Store_Credit_Label::plural()->to_string();

		/* translators: Placeholder: %s - label used to describe account funds, e.g. "Account Funds" */
		$this->title = sprintf( __( '%s increase', 'woocommerce-account-funds' ), $funds_name );

		$this->description = sprintf(
			/* translators: Placeholder: %s - label used to describe account funds, e.g. "Account Funds" */
			__( 'This email is sent to the customer when the %s amount is manually increased.', 'woocommerce-account-funds' ),
			lcfirst( $funds_name )
		);

		add_action( 'wc_account_funds_customer_funds_increased_notification', [ $this, 'trigger' ], 10, 3 );
	...
	public function get_default_heading() {
		return sprintf(
			/* translators: Placeholder: %s - label used to describe account funds, e.g. "Account Funds" */
			__( '%s increased', 'woocommerce-account-funds' ),
	...
	public function get_default_message() {
		return sprintf(
			/* translators: Placeholder: %s - label used to describe account funds, e.g. "Account Funds" */
			__( 'Your %s amount has increased to:', 'woocommerce-account-funds' ),
	...
	public function init_form_fields() {
		...
		$additional_fields = [
			'message' => [
				'title'       => __( 'Message', 'woocommerce-account-funds' ),
				'type'        => 'textarea',
				'desc_tip'    => true,
	...
	protected function get_placeholder_text() : string {
		...
		/* translators: Placeholder: %s - list of available placeholders */
		return sprintf( __( 'Available placeholders: %s.', 'woocommerce-account-funds' ), implode( ', ', array_keys( $this->placeholders ) ) );
	}
