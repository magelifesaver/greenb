...
class WC_Account_Funds_Emails {
	 ...
	 * @since 2.8.0
	 *
	 * @param array<class-string<WC_Email>, WC_Email>|mixed $emails
	 * @return array<class-string<WC_Email>, WC_Email>|mixed
	 */
	public function email_classes( $emails ) {

		if ( ! is_array( $emails ) || isset( $emails['WC_Account_Funds_Email_Account_Funds_Increase'] ) ) {
			return $emails;
		}

		if ( ! class_exists( WC_Account_Funds_Email_Account_Funds_Increase::class ) ) {
			require_once \WC_ACCOUNT_FUNDS_PATH . 'legacy/includes/emails/class-wc-account-funds-email-account-funds-increase.php';
		}

		$emails['WC_Account_Funds_Email_Account_Funds_Increase'] = new WC_Account_Funds_Email_Account_Funds_Increase();
	...
	public function email_styles( $css, $email = null ) {
		...
		if ( ! $email instanceof WC_Email || ! $email->id || false === strpos( $email->id, 'wc_account_funds_' ) ) {
			return $css;
		}
