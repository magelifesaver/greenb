...
class Customer extends User
{
    ...
    {
        $value = $this->get_meta('billing_' . static::PHONE_META_KEY, '');
        if (is_array($value)) {
            $value = current($value);
        }
    ...
    {
        $value = $this->get_meta('shipping_' . static::PHONE_META_KEY, '');
        if (is_array($value)) {
            $value = current($value);
        }
        return is_scalar($value) ? (string) $value : '';
    ...
    {
        $value = $this->get_meta('billing_' . static::EMAIL_META_KEY, $this->get_email());
        if (is_array($value)) {
            $value = current($value);
        }
        return is_scalar($value) ? (string) $value : $this->get_email();
    }
    /**
    ...
    protected function get_address(string $which): Address
    {
        ...
        $address_properties = ['country_code' => $this->get_meta($prefix . static::COUNTRY_CODE_META_KEY, ''), 'administrative_area' => $this->get_meta($prefix . static::ADMINISTRATIVE_AREA_META_KEY, ''), 'locality' => $this->get_meta($prefix . static::LOCALITY_META_KEY, ''), 'postal_code' => $this->get_meta($prefix . static::POSTAL_CODE_META_KEY, ''), 'address_line_1' => $this->get_meta($prefix . static::ADDRESS_LINE_1_META_KEY, ''), 'address_line_2' => $this->get_meta($prefix . static::ADDRESS_LINE_2_META_KEY, ''), 'organization' => $this->get_meta($prefix . static::ORGANIZATION_META_KEY, ''), 'given_name' => $this->get_meta($prefix . static::GIVEN_NAME_META_KEY, ''), 'family_name' => $this->get_meta($prefix . static::FAMILY_NAME_META_KEY, '')];
        // account for invalid data in address meta to avoid type errors
        foreach ($address_properties as $key => $value) {
            if (is_array($value)) {
                $value = current($value);
            }
