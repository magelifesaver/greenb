...
use Kestrel\Account_Funds\Scoped\Kestrel\Fenix\Plugin\Admin\Notices;
use Kestrel\Account_Funds\Scoped\Kestrel\Fenix\Plugin\Assets;
...
use Kestrel\Account_Funds\Scoped\Kestrel\Fenix\Plugin\Integrations;
...
use Kestrel\Account_Funds\Scoped\Kestrel\Fenix\Plugin\Lifecycle\Newsletter_Signup;
use Kestrel\Account_Funds\Scoped\Kestrel\Fenix\Plugin\Privacy;
use Kestrel\Account_Funds\Scoped\Kestrel\Fenix\Plugin\Requirements;
...
abstract class Plugin implements WordPress_Plugin
{
    ...
    /** @var string URL pointing to the plugin vendor homepage */
    protected const VENDOR_URL = 'https://kestrelwp.com/';
    ...
    /**
     * Returns the plugin vendor's URL.
     *
     * @since 1.8.0
     *
     * @return string
    ...
    {
        if (empty(static::VENDOR_URL)) {
            return '';
        }
        return add_query_arg(['utm_source' => 'plugin'], static::VENDOR_URL);
    }
    /**
    ...
    protected function initialize(): void
    {
        ...
        Newsletter_Signup::initialize($this);
        Integrations::initialize($this);
        $this->initialize_privacy_handler();
    }
    /**
     * Initializes the plugin admin handler.
     *
     * @NOTE The admin handler should be the only entry point for all admin functionalities.
     *
     ...
     *
     * This method is provided so that plugin implementations can override it and provide their own REST API handler if needed.
     * Plugins may also override the default handler by providing an 'rest_api.handler' configuration parameter.
     *
     * @since 1.0.0
     *
     * @return void
    ...
    /**
     * Initializes the privacy handler.
     *
     * This method is provided so that plugin implementations can override it and provide their own privacy handler if needed.
     * Plugins may also override the default handler by providing an 'privacy.handler' configuration parameter.
     *
     * @since 1.6.0
     *
     * @return void
     */
    protected function initialize_privacy_handler(): void
    {
        $handler = $this->config()->get('privacy.handler');
        if (!$handler) {
            Privacy::initialize($this);
        } elseif (!is_string($handler) || !class_exists($handler) || !is_a($handler, Privacy::class, \true)) {
            _doing_it_wrong(__METHOD__, 'The privacy handler must be a valid class that extends ' . Privacy::class, '');
        } else {
            $handler::initialize($this);
        }
    }
    ...
    /**
     * Determines if the plugin has been upgraded from a given version.
     *
     * @since 1.7.0
     *
     * @param string $version semver
     * @param string $comparator optional version comparison operator
     * @return bool
     */
    public function has_upgraded_from(string $version, string $comparator = '>='): bool
    {
        $update_history = Lifecycle::get_update_history();
        ...
        foreach ($update_history as $update) {
            ...
            if (empty($update['from']) || !is_string($update['from'])) {
                continue;
            }
            if (version_compare($update['from'], $version, $comparator)) {
                return \true;
            }
        }
        return \false;
    ...
    {
        return ['id' => $this->id(), 'name' => $this->name(), 'locale' => $this->locale(), 'multilingual' => $this->is_multilingual(), 'new_install' => $this->is_new_installation(), 'paths' => ['assets' => $this->assets_path(), 'directory' => $this->absolute_dir_path(), 'file' => $this->absolute_file_path(), 'relative' => $this->relative_file_path(), 'translations' => $this->translations_path(), 'templates' => $this->templates_path()], 'textdomain' => $this->textdomain(), 'urls' => ['assets' => $this->assets_url(), 'base' => $this->base_url(), 'dashboard' => $this->dashboard_url(), 'documentation' => $this->documentation_url(), 'reviews' => $this->reviews_url(), 'sales_page' => $this->sales_page_url(), 'settings' => $this->settings_url(), 'support' => $this->support_url(), 'vendor' => $this->vendor_url()], 'vendor' => $this->vendor(), 'version' => $this->version()];
    }
}
