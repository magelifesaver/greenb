...
final class Notices
{
    ...
    public static function dismiss(?string $notice_id = null, ?int $user_id = null): void
    {
        ...
        if ($user_id && $notice_id && !wp_doing_ajax()) {
            ...
            if ($notice) {
                $notice->dismiss($notice->is_global() ? null : $user_id);
            }
        } elseif (wp_doing_ajax()) {
            // phpcs:ignore
            $notice_id = $_REQUEST['notice'] ?? '';
            $is_global = isset($_REQUEST['global']) && 'yes' === $_REQUEST['global'];
            $notice = null;
            if ($notice_id && is_string($notice_id)) {
                ...
                $notice = Notice::create(['id' => $notice_id, 'global' => $is_global]);
            }
            if (!$user_id) {
            ...
            $dismissed = $notice->dismiss($notice->is_global() ? null : $user_id);
            if (!$dismissed) {
                wp_send_json_error(sprintf('The dismissal action for notice "%s" fired correctly, but WordPress could not dismiss the notice.', $notice_id));
            } else {
                wp_send_json_success(sprintf('%s "%s" is successfully dismissed.', $is_global ? 'Global notice' : 'Notice', $notice_id));
            }
        }
    }
    /**
     ...
     * @param bool $global whether to get global notices or user-specific notices
     * @return string[] notice IDs
     */
    public static function get_dismissed_notices(?int $user_id = null, bool $global = \false): array
    {
        if ($user_id && !$global) {
            $dismissed_notices = get_user_meta($user_id, '_' . self::plugin()->key('dismissed_notices'), \true);
        } elseif (null === $user_id && $global) {
            $dismissed_notices = get_option(self::plugin()->key('dismissed_notices'), []);
        } else {
            $dismissed_notices = [];
        }
    ...
    public static function render(Notice $notice): void
    {
        ...
        $notice_title = $notice->get_title();
        // fallback to the plugin name if title is null
        if (null === $notice_title) {
            $notice_title = self::plugin()->name();
        }
        // if empty string is set as the title, skip formatting and leave blank
        if ('' !== trim($notice_title)) {
            $notice_title = '<strong>' . $notice_title . '</strong> - ';
        }
        $notice_content = $notice_title . $notice->get_content() . $notice_ctas;
        $notice_attributes = array_merge(['data-notice-id' => $notice->get_id(), 'data-plugin-id' => self::plugin()->id(), 'data-is-global' => $notice->is_global() ? 'yes' : 'no'], $notice->get_attributes());
    ...
    private static function render_call_to_actions(Notice $notice): void
    {
        if ($notice->has_call_to_actions()) {
            ...
            foreach ($notice->get_call_to_actions() as $call_to_action) {
                ...
                foreach ($call_to_action->get_classes() as $class) {
    ...
    private function print_scripts(): void
    {
        ...
        ?>',
									notice: notice_id,
									global: notice.dataset.isGlobal === 'yes' ? 'yes' : 'no'
								} )
