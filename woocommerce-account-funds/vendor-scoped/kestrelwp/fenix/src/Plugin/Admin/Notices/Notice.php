 ...
 * @method $this set_global( bool $global )
...
class Notice
{
    ...
    /** @var string|null defaults to plugin name if null */
    ...
    /** @var bool whether this is a global notice (when dismissible, it will record dismissal to an option instead of user meta) */
    ...
    /**
     * Prevents to have the plugin name automatically set as the notice title.
     *
     * @since 1.6.1
     *
     * @return $this
     */
    public function without_title(): Notice
    {
        return $this->set_title('');
    }
    /**
     * Sets the call to actions for the notice.
     *
     * @since 1.0.0
     *
     * @param Call_To_Action[] $call_to_actions
     * @return $this
     */
    public function set_call_to_actions(array $call_to_actions): Notice
    {
        $this->call_to_actions = [];
        foreach ($call_to_actions as $cta) {
            // @phpstan-ignore-next-line type safety check
            if ($cta instanceof Call_To_Action) {
    ...
    /**
     * Determines if the notice is a global notice.
     *
     * Global notices are not user-specific and are stored in an option instead of user meta when dismissible.
     *
     * @since 1.0.0
     *
     * @return bool
     ...
     * @return bool|null
     */
    public function is_dismissed(?int $user_id = null): ?bool
    {
        $is_global = $this->is_global();
        $user_id = null === $user_id && !$is_global ? get_current_user_id() : $user_id;
        if (!$is_global && empty($user_id)) {
            return null;
            // cannot determine dismissal with certainty
        }
        return in_array($this->get_id(), Notices::get_dismissed_notices($user_id ?: null, $is_global), \true);
    }
    /**
     ...
     * @return bool
     */
    public function dismiss(?int $user_id = null): bool
    {
        $is_global = $this->is_global();
        $user_id = null === $user_id && !$is_global ? get_current_user_id() : $user_id;
        if (!$is_global && empty($user_id)) {
        ...
        $dismissed_notices = Notices::get_dismissed_notices($user_id ?: null, $is_global);
        $dismissed_notices[] = $this->get_id();
        if ($is_global) {
            $success = update_option(static::plugin()->key('dismissed_notices'), $dismissed_notices);
        } else {
            $success = (bool) update_user_meta($user_id, '_' . static::plugin()->key('dismissed_notices'), $dismissed_notices);
        }
        if ($success) {
            /**
             * Triggers when an admin notice is dismissed.
             *
             * @since 1.8.0
             *
             ...
             * @param Notice $notice
             */
            do_action(self::plugin()->hook($this->get_id() . '_dismissed'), $this, $user_id);
        }
        return $success;
    }
    /**
     * Restores the notice dismissal status as if it were not dismissed.
     *
     * @since 1.8.0
     *
     * @param int|null $user_id
     * @return bool
     */
    public function restore(?int $user_id = null): bool
    {
        $is_global = $this->is_global();
        $user_id = null === $user_id && !$is_global ? get_current_user_id() : $user_id;
        $dismissed_notices = Notices::get_dismissed_notices($user_id ?: null, $is_global);
        $was_dismissed = \false;
        foreach ($dismissed_notices as $index => $dismissed_notice) {
            if ($dismissed_notice === $this->get_id()) {
                $was_dismissed = \true;
                unset($dismissed_notices[$index]);
                break;
            }
        }
        if (!$was_dismissed) {
            ...
            return update_option(static::plugin()->key('dismissed_notices'), array_values($dismissed_notices));
        } else {
            return (bool) update_user_meta($user_id, '_' . static::plugin()->key('dismissed_notices'), array_values($dismissed_notices));
        }
    }
    /**
