...
if (!\function_exists('collator_create') && !\function_exists('Kestrel\Account_Funds\Scoped\collator_create')) {
...
function filter_duplicate_localizations(array $localizations)
{
    $duplicates = [];
    foreach ($localizations as $locale => $localizedCountries) {
        if ($parentLocale = \Kestrel\Account_Funds\Scoped\CommerceGuys\Addressing\Locale::getParent($locale)) {
