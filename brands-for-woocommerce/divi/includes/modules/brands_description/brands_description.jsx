...
class BRWWL_brands_description extends Component {

  static slug = 'et_pb_brands_description';
  static parameters = ['brand_id',
            'display_title',
            'display_description',
            'thumbnail_display',
            'thumbnail_width',
  ...
  componentDidUpdate(oldProps) {
      var update = false;
      BRWWL_brands_description.parameters.forEach(key => {
  ...
  componentDidMount() {
    ...
    body.append('action', 'brbrands_description');
      ...
      .then(
        (result) => {
          if( typeof(result) === 'undefined' || ! result ) {
              this.htmlstate = (<div style={{padding:"2em 0", background: "#6c2eb9", color: "#fff", fontSize: "12px", fontWeight: "600", verticalAlign: "middle", textAlign: "center", borderRadius: "1em"}}><h3 style={{color: "#000", textShadow: "1px 0px white, -1px 0px white, 0px 1px white, 0px -1px white", fontWeight: "900"}}>Product Brands Description</h3>Brands info not displayed in Builder</div>);
              this.setState({
                isLoaded: true
              });
          } else {
...
export default BRWWL_brands_description;
