...
class WOOGC_WC_Cart_Session {
     ...
     */
    public function set_cart( WOOGC_WC_Cart $cart ) {
        $this->cart = $cart;
    }
	 ...
	 */
	public function init( $reload_session_data =    FALSE) {
        
        if ( $reload_session_data  )
    ...
    public function _refresh_session()
        {
	...
	public function get_cart_from_session() {
		...
		if ( is_array( $cart ) ) {
			...
			foreach ( $cart as $key => $values ) {
                ...
                $values =   apply_filters( 'woogc/get_cart_from_session/cart_item/values', $values, $key, $cart );
				...
				if ( ! empty( $product ) && $product->exists() && $values['quantity'] > 0 ) {
                    ...
                    if ( isset ( $values['blog_id'] ) )
                        $product->add_meta_data ( 'blog_id', $values['blog_id'] );
					...
					} elseif ( ! empty( $values['data_hash'] ) && ! hash_equals( $values['data_hash'], wc_get_cart_item_data_hash( $product ) ) && ! apply_filters ( 'woogc/get_cart_from_session/validate_hash', FALSE, $values ) ) { // phpcs:ignore PHPCompatibility.PHP.NewFunctions.hash_equalsFound
						$update_cart_session = true;
                ...
                foreach ( $sites  as  $site ) 
                    {
                        ...
                        update_user_meta(
                                    get_current_user_id(), '_woocommerce_persistent_cart_' . $site->blog_id, array(
