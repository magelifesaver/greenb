...
namespace WooGC\Checkout\Helpers;
 ...
 */
final class WooGC_ReserveStock {
	...
	public function reserve_stock_for_order( $order, $minutes = 0 ) {
		...
		try {
			...
			foreach ( $items as $item ) {
				...
				$quantity = isset( $rows[ $managed_by_id ] ) ? $rows[ $managed_by_id ] + $item_quantity : $item_quantity;
                $this->reserve_stock_for_product( $managed_by_id, $quantity, $order, $minutes );
