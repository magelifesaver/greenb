 ...
 */
class WooGC_Download_Handler extends WC_Download_Handler {
	...
	public static function init() {
		if ( isset( $_GET['download_file'], $_GET['order'] ) && ( isset( $_GET['email'] ) || isset( $_GET['uid'] ) ) && ( isset( $_GET['sid'] ) ) ) { // WPCS: input var ok, CSRF ok.
			
            global $WooGC;
                
            //remove the other hooks for WC_Cart_Session
            $WooGC->functions->remove_class_action('init', 'WC_Download_Handler', 'download_product');
            
            add_action( 'init', array( __CLASS__, 'download_product' ) );

            $WooGC->functions->remove_class_action('woocommerce_download_file_redirect', 'WC_Download_Handler', 'download_file_redirect');
            $WooGC->functions->remove_class_action('woocommerce_download_file_xsendfile', 'WC_Download_Handler', 'download_file_xsendfile');
            $WooGC->functions->remove_class_action('woocommerce_download_file_force', 'WC_Download_Handler', 'download_file_force');
	...
	public static function download_product() {
		...
		$sid_id     = absint( $_GET['sid'] );  
        
        self::$blog_id  =   $sid_id;
        
        switch_to_blog( $sid_id );
	...
	public static function parse_file_path( $file_path ) {
        ...
        $wp_uploads    =   self::_wp_upload_url();
    ...
    private static function _wp_upload_url()
        {
