 ...
 * @version     2.1.0
 */
class WooGC_Report_Customer_List extends WP_List_Table {
	...
	public function column_default( $user, $column_name ) {
		switch ( $column_name ) {
			...
			case 'last_order':
				...
				if ( ! empty( $orders ) ) {
					$order = $orders[0];
					return '<a href="' . network_admin_url( 'post.php?post=' . $order->get_id() . '&action=edit' ) . '">' . _x( '#', 'hash before order number', 'woocommerce' ) . $order->get_order_number() . '</a> &ndash; ' . wc_format_datetime( $order->get_date_created() );
				} else {
					return '-';
				}
			...
			case 'wc_actions':
				...
				?><p>
					...
					$actions['edit'] = array(
						'url'    => network_admin_url( 'user-edit.php?user_id=' . $user->ID ),
	...
	public function prepare_items() {
        ...
        $_network_sites  =   $WooGC->functions->get_gc_sites( TRUE, 'global_reports' );
        $network_sites  =   array();
