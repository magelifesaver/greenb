...
class WooGC_Admin_Report {
	...
	public function get_order_report_data( $args = array() ) {
        ...
        if  ( !isset($network_sites) )
            {
                global $blog_id; 
                
                $network_sites[]    =   get_blog_details( $blog_id );
            }
        ...
        foreach( $network_sites as  $site) 
            {
		        ...
		        foreach ( ( $data + $where ) as $raw_key => $value ) {
                    ...
                    if ( isset ( $value['JOIN'] )   &&  $value['JOIN']  === FALSE )
                        continue;
			        ...
			        switch ( $type ) {
				        case 'wc_order_stats':
                            $joins[ "meta_{$key}" ] = "{$join_type} JOIN {$wpdb->prefix}wc_order_stats AS OS ON ( posts.ID = OS.order_id )";
                            break;
                        case 'wc_order_operational_data':
                            $joins[ "meta_{$key}" ] = "{$join_type} JOIN {$wpdb->prefix}wc_order_operational_data AS OOD ON ( posts.ID = OOD.order_id )";
                            break;
                        case 'meta':
		        ...
		        if ( $filter_range ) {
			        $query['where'] .= "
				        AND 	posts.date_created_gmt >= '" . date( 'Y-m-d H:i:s', $this->start_date ) . "'
				        AND 	posts.date_created_gmt < '" . date( 'Y-m-d H:i:s', strtotime( '+1 DAY', $this->end_date ) ) . "'
			        ";
		        }
        ...
        //create the query
        $network_select =   "SELECT * FROM";
        if ( isset($network_query['select']) )
            $network_select =   $network_query['select'];
        
        $query  =   $network_select . " ( (" . implode( ") UNION ALL (", $main_query ) . ") ) AS Q";
        if ( $order_by ) {
	...
	public function sales_sparkline( $id = '', $days = 7, $type = 'sales' ) {
		...
		if ( $id ) {
			...
			$data = $this->get_order_report_data(
				array(
					...
					'group_by'     => 'YEAR(posts.date_created_gmt), MONTH(posts.date_created_gmt), DAY(posts.date_created_gmt)',
		...
		$sparkline_data = array_values( $this->prepare_chart_data( $data, 'date_created_gmt', 'sparkline_value', $days - 1, strtotime( 'midnight -' . ( $days - 1 ) . ' days', current_time( 'timestamp' ) ), 'day' ) );
	...
	public function calculate_current_range( $current_range ) {
		...
		switch ( $this->chart_groupby ) {
			...
			case 'day':
				$this->group_by_query = 'YEAR(posts.date_created_gmt), MONTH(posts.date_created_gmt), DAY(posts.date_created_gmt)';
				$this->chart_interval = absint( ceil( max( 0, ( $this->end_date - $this->start_date ) / ( 60 * 60 * 24 ) ) ) );
				$this->barwidth       = 60 * 60 * 24 * 1000;
				break;

			case 'month':
				$this->group_by_query = 'YEAR(posts.date_created_gmt), MONTH(posts.date_created_gmt)';
