 ...
 * @version     2.1.0
 */
class WooGC_Report_Customers extends WooGC_Admin_Report {
	...
	public function customers_vs_guests() {
		...
		$customer_order_totals = $this->get_order_report_data(
			array(
                ...
                'network_query' =>  array(
                                            'select'    =>  'SELECT COUNT(total_orders) as total_orders FROM'
                                            )
			)
		);
	...
	public function output_report() {
        ...
        $_network_sites  =   $WooGC->functions->get_gc_sites( TRUE );
        $network_sites  =   array();
        foreach ( $network_sites as  $site )
            {
                $network_sites[]    =   $site->blog_id;   
            }
	...
	public function get_main_chart() {
		...
		$customer_orders = $this->prepare_chart_data( $customer_orders, 'date_created_gmt', 'total_orders', $this->chart_interval, $this->start_date, $this->chart_groupby );
		$guest_orders    = $this->prepare_chart_data( $guest_orders, 'date_created_gmt', 'total_orders', $this->chart_interval, $this->start_date, $this->chart_groupby );
