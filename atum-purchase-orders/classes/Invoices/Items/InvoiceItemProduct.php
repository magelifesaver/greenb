<?php
/**
 * The class for the Invoice Item Product objects
 *
 * @package         AtumPO\Invoices
 * @subpackage      Items
 * @author          BE REBEL - https://berebel.studio
 * @copyright       ©2025 Stock Management Labs™
 *
 * @since           0.9.6
 */

namespace AtumPO\Invoices\Items;

defined( 'ABSPATH' ) || die;

use Atum\Components\AtumOrders\Items\AtumOrderItemProduct;
use AtumPO\Abstracts\AtumPOOrders\Items\AtumPOOrderItemProductTrait;
use AtumPO\Invoices\Models\InvoiceItem;


class InvoiceItemProduct extends AtumOrderItemProduct {

	/**
	 * The Invoice item data array
	 *
	 * @var array
	 */
	protected $extra_data = array(
		'product_id'   => 0,
		'variation_id' => 0,
		'po_item_id'   => 0,
		'quantity'     => 1,
		'subtotal'     => 0,
		'subtotal_tax' => 0,
		'total'        => 0,
		'total_tax'    => 0,
		'taxes'        => array(
			'subtotal' => array(),
			'total'    => array(),
		),
	);

	/**
	 * Meta keys reserved for internal use
	 *
	 * @var array
	 */
	protected $internal_meta_keys = array( '_product_id', '_variation_id', '_po_item_id', '_qty', '_line_subtotal', '_line_subtotal_tax', '_line_total', '_line_tax', '_line_tax_data', '_tax_config', '_discount_config' );

	/**
	 * Load the Invoice item
	 *
	 * @since 0.9.6
	 */
	protected function load() {

		/* @noinspection PhpParamsInspection PhpUnhandledExceptionInspection */
		$this->atum_order_item_model = new InvoiceItem( $this );

		if ( ! $this->atum_order_id ) {
			$this->atum_order_id = $this->atum_order_item_model->get_atum_order_id();
		}

		$this->read_meta_data();

	}

	/**
	 * Saves an item's meta data to the database
	 * Runs after both create and update, so $id will be set
	 *
	 * @since 0.9.6
	 */
	public function save_item_data() {

		$save_values = (array) apply_filters( 'atum/purchase_orders_pro/invoice_item_product/save_data', array(
			'_product_id'        => $this->get_product_id( 'edit' ),
			'_variation_id'      => $this->get_variation_id( 'edit' ),
			'_po_item_id'        => $this->get_po_item_id( 'edit' ),
			'_qty'               => $this->get_quantity( 'edit' ),
			'_line_subtotal'     => $this->get_subtotal( 'edit' ),
			'_line_subtotal_tax' => $this->get_subtotal_tax( 'edit' ),
			'_line_total'        => $this->get_total( 'edit' ),
			'_line_tax'          => $this->get_total_tax( 'edit' ),
			'_line_tax_data'     => $this->get_taxes( 'edit' ),
			'_discount_config'   => maybe_unserialize( $this->get_meta( '_discount_config' ) ),
			'_tax_config'        => maybe_unserialize( $this->get_meta( '_tax_config' ) ),
		), $this );

		$this->atum_order_item_model->save_meta( $save_values );

	}

	/**
	 * Get invoice item type.
	 *
	 * @since 0.9.6
	 *
	 * @return string
	 */
	public function get_type() {
		return 'invoice_item';
	}

	use AtumPOOrderItemProductTrait;

}
