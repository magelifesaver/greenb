<?php
/**
 * View for the PO's fee items
 *
 * @since 0.9.17
 *
 * @var \AtumPO\Models\POExtended             $po
 * @var \AtumPO\Invoices\Items\InvoiceItemFee $invoice_item
...
$missing_po_item = ! $po->get_item( $invoice_item->get_po_item_id(), 'fee' );
?>
<tr class="invoice-item-row fee<?php echo $missing_po_item ? ' missing-item atum-tooltip' : '' ?>" data-invoice="<?php echo esc_attr( $invoice->get_id() ) ?>"
...
>
	...
	<td class="invoice-item-info">
		...
		<?php // Fee name.
		$fee_name = $invoice_item->get_name() ?: __( 'Fee', ATUM_PO_TEXT_DOMAIN ); ?>
		<span class="name">
			...
			<template id="edit-name-<?php echo esc_attr( $item_id ) ?>">
				<input type="text" autocomplete="off" value="<?php echo esc_attr( $fee_name ) ?>" class="meta-value">
			</template>

			<input type="hidden" name="atum_order_item_name[<?php echo absint( $item_id ) ?>]" value="<?php echo esc_attr( $fee_name ) ?>">
		</span>
	</td>

	<td colspan="3">
		&nbsp;
		<input type="hidden" class="atum_order_item_id" name="atum_order_fee_id[]" value="<?php echo absint( $item_id ) ?>">
	</td>
