<?php
/**
 * View for the Purchase Orders' PDF (Template 2)
 *
 * @since 0.9.7
 *
...
if ( $template_color ) : ?>
	<style>
		...
		.content-address h4, .content-address h5, .content-lines .po-li-head th, footer .label {
			background-color: <?php echo esc_html( $template_color ) ?> !important;
		}

		.address {
			border-bottom-color: <?php echo esc_html( $template_color ) ?>;
		}

		.content-totals .subtotal td {
...
<div class="main-page<?php echo $po->get_debug_mode() ? ' is-debug' : '' ?>">
	...
	<?php if (
		( ( array_key_exists( 'requisitioner', $template_fields ) && 'yes' === $template_fields['requisitioner'] ) || ( array_key_exists( 'delivery_terms', $template_fields ) && 'yes' === $template_fields['delivery_terms'] ) ) &&
		( ( $requires_requisition && $po->requisitioner ) || $po->delivery_terms )
	) : ?>
		<div class="po-wrapper content-address content-address__extra">
	...
	<div class="po-wrapper content-lines">
		<table>
				...
				<?php if ( ! empty( $line_items_shipping ) ) : ?>
					...
					<?php foreach ( $line_items_shipping as $item_id => $item ) : ?>
						<tr class="po-line content-shipping">
							...
							<?php $shipping_total = $item->get_total(); ?>
			...
			<tbody class="content-totals">
				...
				<tr class="subtotal">
					<td colspan="<?php echo esc_attr( $total_text_colspan - 1 ) ?>"></td>
					<td class="label">
	...
	<?php if ( array_key_exists( 'description', $template_fields ) && 'yes' === $template_fields['description'] && $description ) : ?>
		<footer>
			<div class="label">
				<?php esc_html_e( 'Notes', ATUM_PO_TEXT_DOMAIN ) ?>
			</div>
			<div class="po-content">
				<?php
				echo wp_kses_post( html_entity_decode( apply_filters( 'the_content', $description ), ENT_COMPAT, 'UTF-8' ) ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound
				?>
			</div>
		</footer>
	<?php endif; ?>
