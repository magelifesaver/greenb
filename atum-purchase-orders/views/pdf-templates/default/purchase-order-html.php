<?php
/**
 * View for the Purchase Orders' PDF (Default template)
 *
 * @since 0.9.7
 *
...
if ( $template_color ) :
	...
	<style>
		...
		.content-address h4, .content-address h5, .content-lines .po-li-head {
			background-color: <?php echo esc_html( $template_color ) ?>;
		}
...
<div class="main-page<?php echo $po->get_debug_mode() ? ' is-debug' : '' ?>">
	...
	<div class="po-wrapper content-header">
		<div class="float-left">
			...
			<?php if ( $company_logo ) : ?>
			<div class="logo"><?php echo wp_get_attachment_image( $company_logo, 'full', false, [ 'style' => 'max-width:300px;height:auto;' ] ) ?></div>
			<?php endif; ?>
			...
			<tbody class="po-lines">
				<?php foreach ( $po->get_items() as $item ) :
					...
					<tr class="po-line">
						...
						<?php endif; ?>
						<td class="total"><?php echo $po->format_price( (float) $item->get_total() + (float) $tax_item_total ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></td>
					</tr>
				<?php endforeach; ?>
