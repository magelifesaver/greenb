...
echo sprintf( esc_html__( 'Order link: %s', 'woocommerce-order-status-manager' ), esc_url( admin_url( 'post.php?post=' . $order->get_id() . '&action=edit' ) ) ) . "\n";

if ( $date_created = $order->get_date_created() ) {

	/* translators: Placeholders: %s - order date */
	echo sprintf( esc_html__( 'Order date: %s', 'woocommerce-order-status-manager' ), esc_html( date_i18n( __( 'jS F Y', 'woocommerce-order-status-manager' ), $date_created->getTimestamp() ) ) ) . "\n";
}
...
echo esc_html__( 'Customer details', 'woocommerce-order-status-manager' ) . "\n";
