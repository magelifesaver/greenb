 ...
 * File Path: /aaa-order-workflow/includes/indexers/class-aaa-oc-payment-indexer.php
...
public static function sync_payment_totals( $post_id ) {
    ...
    // 🔹 NEW: determine real_payment_method from non-zero amounts
    $amount_map = [
    ...
    if ( count($nonzero) === 1 ) {
        ...
        // pick the method with the largest amount
        $max = max($nonzero);
        $real_payment_method = array_search($max, $nonzero, true);
    } else {
        // no payments recorded yet — fall back to original gateway id (as-is)
