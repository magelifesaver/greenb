<?php
/**
 * File Path: /aaa-order-workflow/includes/printing/class-aaa-oc-printing.php
 *
 * Purpose:
    ...
    public static function render_print_buttons( $order_id ) {
        ...
        <div class="aaa-oc-print-buttons-wrap" style="margin:6px 0;">
            ...
            <div class="aaa-lpm-buttons">
                ...
                <?php
                // Quick-jump scheduling buttons
                if ( $current_status === 'lkd-packed-ready' ) : ?>
                    <button type="button"
                        class="button-modern schedule-delivery-btn"
                        title="Move order directly to Scheduled"
                        onclick="try{aaaOcCloseModal();}catch(e){}; aaaOcChangeOrderStatus(<?php echo intval( $order_id ); ?>, 'scheduled');">
                        Schedule <br> Delivery
                    </button>
                <?php
                elseif ( $current_status === 'scheduled' ) : ?>
                    <button type="button"
                        class="button-modern schedule-delivery-btn"
                        title="Move order back to Packed & Ready"
                        onclick="try{aaaOcCloseModal();}catch(e){}; aaaOcChangeOrderStatus(<?php echo intval( $order_id ); ?>, 'lkd-packed-ready');">
                        Set <br> Packed & Ready
                    </button>
                <?php endif; ?>
            </div>
