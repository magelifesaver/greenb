<?php
/**
 * Forecast queue admin tab.
 *
 * Displays counts, queue rows and provides buttons to schedule
 * background jobs such as queuing all enabled products, processing
 * the forecast queue and processing the PO queue. All admin-post
 * actions are nonce protected. This page is accessed via the core
 * settings page tab: ?page=aaa-oc-core-settings&tab=aaa-oc-forecast-queue.
 *
...
// Determine which status to filter by. Default to pending.
$filter = isset( $_GET['queue_status'] ) ? sanitize_key( wp_unslash( $_GET['queue_status'] ) ) : 'pending';
if ( ! in_array( $filter, [ 'pending', 'processing', 'done', 'all' ], true ) ) {
    $filter = 'pending';
}
...
<div class="wrap">
    ...
    <?php if ( ! empty( $_GET['aaa_oc_forecast_repaired'] ) ) : ?>
        <div class="notice notice-success"><p><?php esc_html_e( 'Forecast tables repaired.', 'aaa-oc' ); ?></p></div>
    <?php endif; ?>
    ...
    <?php if ( ! empty( $_GET['aaa_oc_forecast_process_scheduled'] ) ) : ?>
        <div class="notice notice-success"><p><?php esc_html_e( 'Forecast queue processing has been scheduled to run in the background.', 'aaa-oc' ); ?></p></div>
    <?php endif; ?>
    ...
    <form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" style="display:inline-block;">
        ...
        <button type="submit" class="button button-primary"><?php esc_html_e( 'Process Forecast Queue (Scheduled)', 'aaa-oc' ); ?></button>
    </form>
