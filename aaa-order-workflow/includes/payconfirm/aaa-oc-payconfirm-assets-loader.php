...
/**
 * Important:
 * - Do NOT force-enable debug here.
 * - We only respect the global AAA_OC_PAYCONFIRM_DEBUG defined in the main loader.
 * - If that constant is never defined, logs stay silent by default.
 */
if ( ! defined( 'AAA_OC_PAYCONFIRM_DEBUG' ) ) {
	define( 'AAA_OC_PAYCONFIRM_DEBUG', false );
}
...
class AAA_OC_PayConfirm_Assets {
	...
	public static function enqueue( $hook ) {
		...
		$ver        = file_exists( $fs_path )
			? (string) filemtime( $fs_path )
			: ( defined( 'AAA_OC_PAYCONFIRM_VERSION' ) ? AAA_OC_PAYCONFIRM_VERSION : '1.0.0' );
		...
		// Only log if the global PayConfirm debug flag is explicitly enabled.
