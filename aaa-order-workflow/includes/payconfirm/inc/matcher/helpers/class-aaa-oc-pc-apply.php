<?php
/**
 * File: /wp-content/plugins/aaa-order-workflow/includes/payconfirm/inc/matcher/helpers/class-aaa-oc-pc-apply.php
 * Purpose: Apply a match (meta writes, mirrors, index upsert, notes) + logging.
 */
...
class AAA_OC_PC_Apply {
	...
	public static function apply( $order_id, $post_id, array $f, $conf, $why ) {
			...
			if ( $uid = $order->get_user_id() ) {
				...
				if ( $method_key && $alias !== '' ) {
					...
					if ( isset( $map[ $method_key ] ) ) {
						// Back-compat: previously stored as string; normalize to array.
						$list = is_array( $map[ $method_key ] ) ? $map[ $method_key ] : [ (string) $map[ $method_key ] ];
					}
		...
		// mirrors (identical to board-payment-save.js mapping)
		...
		// payment index upsert (+ optional sync)
		...
		if ( $ok && class_exists( 'AAA_OC_Indexing' ) )       { ( new AAA_OC_Indexing() )->index_order( $order_id ); }
