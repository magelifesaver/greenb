...
class AAA_OC_PayConfirm_Bulk {
	...
	public static function handle_action( $redirect_url, $action, $post_ids ) {
		...
		$c_ok = $c_partial = $c_unmatched = $c_err = 0;

		foreach ( (array) $post_ids as $post_id ) {
			$post = get_post( $post_id );
			if ( ! $post || $post->post_type !== 'payment-confirmation' ) { $c_err++; continue; }

			if ( $action === 'aaa_oc_payconfirm_parse_publish' ) {
	...
	public static function admin_notice() {
		...
		if ( ! $screen || $screen->id !== 'edit-payment-confirmation' ) return;
