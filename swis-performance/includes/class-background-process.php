<?php
/**
 * Background Process class to be extended further for various functions.
 *
...
abstract class Background_Process extends Async_Request {
	...
	public function push_to_queue( $data ) {
		...
		$exists = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM $wpdb->swis_queue WHERE page_url = %s AND queue_name = %s", $data, $this->active_queue ) );
		if ( empty( $exists ) ) {
			$to_insert = array(
				'page_url'   => $data,
				'params'     => $params,
				'queue_name' => $this->active_queue,
			);
			$wpdb->insert( $wpdb->swis_queue, $to_insert );
		}
	}
	...
	public function update( $id, $data = array() ) {
		if ( ! empty( $id ) ) {
			...
			$wpdb->get_row( $wpdb->prepare( "UPDATE $wpdb->swis_queue SET attempts=attempts+1,params=%s WHERE id = %d", $data['params'], $id ) );
		}
	}
	...
	public function count_queue() {
		global $wpdb;
		return (int) $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM $wpdb->swis_queue WHERE queue_name = %s", $this->active_queue ) );
	}
	...
	protected function get_batch() {
		global $wpdb;
		$batch = $wpdb->get_results( $wpdb->prepare( "SELECT id,page_url,attempts,params FROM $wpdb->swis_queue WHERE queue_name = %s LIMIT %d", $this->active_queue, $this->limit ), ARRAY_A );
		if ( empty( $batch ) ) {
			return array();
		}
	...
	protected function handle() {
		...
		do {
			...
			foreach ( $batch as $key => $value ) {
				...
				$value['params'] = maybe_unserialize( $value['params'] );
	...
	public function cancel_process() {
		global $wpdb;
		$wpdb->query( $wpdb->prepare( "DELETE from $wpdb->swis_queue WHERE queue_name = %s", $this->active_queue ) );
