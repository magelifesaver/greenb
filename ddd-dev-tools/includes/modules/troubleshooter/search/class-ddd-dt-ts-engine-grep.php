...
class DDD_DT_TS_Engine_Grep {
    public static function search( $roots, array $args ): array {
        if ( ! ddd_dt_ts_command_exists( 'grep' ) ) {
            return [ 'ok' => false, 'error' => 'System grep not available (or shell_exec disabled).' ];
        }
        if ( ! ddd_dt_ts_is_proc_open_enabled() ) {
            return [ 'ok' => false, 'error' => 'proc_open is not available on this server.' ];
        }
        ...
        $items = [];
        $total_matches = 0;
        $warnings = [ 'Max file size limit is not enforced with grep. Use PHP or rg for strict max-file-size.' ];
        ...
        foreach ( (array) $roots as $root ) {
            ...
            $cmd = self::build_cmd( $root, $term, $ignore_case, $whole_word, $regex, $files_only, $exts, $exclude );
    ...
    private static function build_cmd( string $root, string $term, bool $ignore_case, bool $whole_word, bool $regex, bool $files_only, array $exts, array $exclude ): string {
        $parts = [ 'grep', '--color=never', '--binary-files=without-match', '-I' ];
        if ( is_dir( $root ) ) {
            $parts[] = '-r';
        }
        $parts[] = $files_only ? '-l' : '-n';
        if ( $ignore_case ) {
            $parts[] = '-i';
        }
        if ( $whole_word ) {
        ...
        foreach ( (array) $exts as $ext ) {
            ...
            if ( $ext !== '' ) {
                $parts[] = '--include=' . escapeshellarg( '*.' . $ext );
            }
        }
        foreach ( (array) $exclude as $dir ) {
            $dir = preg_replace( '/[^a-zA-Z0-9._-]+/', '', (string) $dir );
            if ( $dir !== '' ) {
                $parts[] = '--exclude-dir=' . escapeshellarg( $dir );
            }
        }

        $parts[] = '-e';
        $parts[] = escapeshellarg( $term );
    ...
    private static function run_cmd( string $cmd, bool $files_only, array &$items, int &$total_matches, int $max_results, float $start, int $max_ms, array &$warnings ): array {
        ...
        if ( ! is_resource( $proc ) ) {
            return [ 'ok' => false, 'error' => 'Failed to start grep process.' ];
        }
        ...
        while ( ( $line = fgets( $pipes[1] ) ) !== false ) {
            ...
            if ( $total_matches >= $max_results || ( ( microtime( true ) - $start ) * 1000 ) >= $max_ms ) {
                $stop = true;
                break;
            }
        }
        ...
        if ( $err !== '' ) {
            $warnings[] = sanitize_text_field( $err );
        }
